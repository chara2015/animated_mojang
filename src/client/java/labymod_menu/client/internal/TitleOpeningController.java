...
public final class TitleOpeningController {
	...
	public static void playOpening() {
		if (openingPlayed || !canStartOpening()) {
			return;
		}
	...
	public static boolean shouldHideTitleWidgets() {
		return LabyModMenuConfig.isMinecraftTitleAnimationEnabled() && isOpeningPlaying()
	...
	public static float getTitleWidgetAlpha() {
		if (!LabyModMenuConfig.isMinecraftTitleAnimationEnabled() || !isOpeningPlaying()) {
			return 1.0F;
		}
	...
	public static void renderAnimatedMinecraftTitle(GuiGraphicsExtractor graphics, float alpha) {
		if (openingPlayed && LabyModMenuConfig.isMinecraftTitleAnimationEnabled()) {
	...
	public static void renderMinecraftEditionWithMenu(GuiGraphicsExtractor graphics, float alpha) {
		if (openingPlayed && LabyModMenuConfig.isMinecraftTitleAnimationEnabled() && !shouldHideTitleWidgets()) {
	...
	private static void updateCameraInterpolation(long now, Screen screen) {
		...
		float progress = smoothWorldReturnTransition ? smootherStep(raw)
				: cubicBezier(raw, CURVE_X1, CURVE_Y1, CURVE_X2, CURVE_Y2);
	...
	private static boolean canStartOpening() {
		Minecraft minecraft = Minecraft.getInstance();
		return minecraft.getOverlay() == null && minecraft.screen instanceof TitleScreen
				&& (!LabyModMenuConfig.isMojangLogoAnimationEnabled()
						|| !MojangAnimation.hasStarted() || MojangAnimation.isFinished());
	}
