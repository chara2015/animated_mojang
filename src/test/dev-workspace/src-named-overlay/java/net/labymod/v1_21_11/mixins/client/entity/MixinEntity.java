...
/* JADX INFO: loaded from: LabyMod-4-v1_21_11-named.jar:net/labymod/v1_21_11/mixins/client/entity/MixinEntity.class */
@Mixin({Entity.class})
...
public abstract class MixinEntity implements net.labymod.api.client.entity.Entity {
    ...
    private void labyMod$fireEntityEyeHeightEventEyeY(InsertInfoReturnable<Double> info) {
        EntityEyeHeightEvent event = Laby.fireEvent(new EntityEyeHeightEvent(this, this.eyeHeight));
    ...
    private void labyMod$oldRange(InsertInfoReturnable<Float> info) {
        ...
        RangeOldAnimation animation = oldAnimationRegistry.get("range");
    ...
    public void setNameTagType(TagType type) {
        String string = getNameTagMatch();
        if (string != null) {
            getTags().remove(string);
        }
        if (type != null) {
            getTags().add("labymod-nametag:" + String.valueOf(type));
        }
    }
    ...
    public ResourceLocation entityId() {
        return (ResourceLocation) this.labyMod$entityId.get();
    }
}
