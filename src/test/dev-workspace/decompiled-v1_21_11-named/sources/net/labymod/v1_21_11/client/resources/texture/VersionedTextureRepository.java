...
/* JADX INFO: loaded from: LabyMod-4-v1_21_11-named.jar:net/labymod/v1_21_11/client/resources/texture/VersionedTextureRepository.class */
@Singleton
...
public class VersionedTextureRepository extends AbstractTextureRepository implements TextureRepository {
    ...
    public void onDevTick() {
        super.onDevTick();
        for (PathResourceLocation pathResourceLocation : this.labyAPI.renderPipeline().resources().resourceLocationFactory().getCachedResourceLocations().values()) {
            if (pathResourceLocation instanceof PathResourceLocation) {
                PathResourceLocation path = pathResourceLocation;
                GameTextureManager textureManager = getTextureManager();
                Identifier minecraftLocation = (Identifier) pathResourceLocation.getMinecraftLocation();
                boolean hasResource = textureManager.hasResource((ResourceLocation) pathResourceLocation.getMinecraftLocation());
