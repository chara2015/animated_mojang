...
import net.labymod.v1_19_4.client.gui.ScreenAccessor;
...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_19_4/mixins/client/screen/MixinScreen.class */
@Mixin({etd.class})
public abstract class MixinScreen implements FileDropHandler, ScreenAccessor {
    ...
    @Shadow
    protected abstract void shadow$a(ehe eheVar, @Nullable uf ufVar, int i, int i2);

    @Insert(method = {"init(Lnet/minecraft/client/Minecraft;II)V"}, at = @At("HEAD"))
    public void fireScreenInitEvent(emh mc, int width, int height, InsertInfo ci) {
        Laby.fireEvent(new VersionedScreenInitEvent(this));
    }

    @Insert(method = {"renderBackground(Lcom/mojang/blaze3d/vertex/PoseStack;)V"}, at = @At("HEAD"), cancellable = true)
    public void renderThemeBackground(ehe stack, InsertInfo ci) {
        MutableMouse mouse = Laby.labyAPI().minecraft().mouse();
        Laby.references().renderEnvironmentContext().screenContext().runInContext(((VanillaStackAccessor) stack).stack(), mouse, emh.N().aw(), context -> {
            renderThemeBackground(context, ci);
        });
    }

    @Insert(method = {"renderDirtBackground(Lcom/mojang/blaze3d/vertex/PoseStack;)V"}, at = @At("HEAD"), cancellable = true)
    public void renderThemeDirtBackground(ehe stack, InsertInfo ci) {
        MutableMouse mouse = Laby.labyAPI().minecraft().mouse();
        Laby.references().renderEnvironmentContext().screenContext().runInContext(((VanillaStackAccessor) stack).stack(), mouse, emh.N().aw(), context -> {
            renderThemeBackground(context, ci);
        });
    ...
    @Insert(method = {"renderBackground(Lcom/mojang/blaze3d/vertex/PoseStack;)V"}, at = @At(value = "INVOKE", target = "Lnet/minecraft/client/gui/screens/Screen;fillGradient(Lcom/mojang/blaze3d/vertex/PoseStack;IIIIII)V", shift = At.Shift.BEFORE), cancellable = true)
    private void labyMod$disableMenuBackground(ehe poseStack, InsertInfo ci) {
    ...
    public void renderComponentHoverEffect(Stack stack, Style style, int x, int y) {
        shadow$a((ehe) stack.getProvider().getPoseStack(), (uf) style, x, y);
    }

    @Override // net.labymod.v1_19_4.client.gui.ScreenAccessor
    public void setMinecraft(emh minecraft) {
        this.e = minecraft;
    }
    ...
    public List<Object> getWrappedWidgets() {
        ...
        for (epl child : this.p) {
            wrappedWidgets.add(asVersionedWidget(child));
        }
        return wrappedWidgets;
    }

    private VersionedWidget asVersionedWidget(epl child) {
        ...
        if (child instanceof epk) {
            epk container = (epk) child;
            for (epl entry : container.i()) {
