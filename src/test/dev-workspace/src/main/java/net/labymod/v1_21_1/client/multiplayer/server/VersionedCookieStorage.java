...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_1/client/multiplayer/server/VersionedCookieStorage.class */
public class VersionedCookieStorage implements CookieStorage {
    private final fzy transferState;

    public VersionedCookieStorage(fzy transferState) {
    ...
    public void setCookie(ResourceLocation cookieId, byte[] data) {
        this.transferState.a().put((akr) cookieId.getMinecraftLocation(), data);
    }
    ...
    public Map<ResourceLocation, byte[]> cookies() {
        Map<akr, byte[]> vanillaCookies = this.transferState.a();
        Map<ResourceLocation, byte[]> cookies = new HashMap<>(vanillaCookies.size());
        for (Map.Entry<akr, byte[]> entry : vanillaCookies.entrySet()) {
