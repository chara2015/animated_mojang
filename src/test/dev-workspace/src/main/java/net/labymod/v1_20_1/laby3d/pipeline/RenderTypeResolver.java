...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_20_1/laby3d/pipeline/RenderTypeResolver.class */
public final class RenderTypeResolver {
    ...
    private static final Function<LevelMaterial, fkf> DYNAMIC_TYPES = MEMOIZE.memoize(levelMaterial -> {
        MinecraftRenderState renderState = LINKER.get(levelMaterial.renderState());
        return fkf.a(getRenderTypeName(levelMaterial.id()), renderState.getVertexFormat(), renderState.getDrawMode(), 65536, renderState.affectsCrumbling(), renderState.sortOnUpload(), renderState.create(levelMaterial, false));
    });
    ...
    public fkf resolve(Material material) {
