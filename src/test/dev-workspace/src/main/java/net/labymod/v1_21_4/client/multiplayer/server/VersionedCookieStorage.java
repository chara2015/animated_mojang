...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_4/client/multiplayer/server/VersionedCookieStorage.class */
public class VersionedCookieStorage implements CookieStorage {
    private final ggt transferState;

    public VersionedCookieStorage(ggt transferState) {
    ...
    public void setCookie(ResourceLocation cookieId, byte[] data) {
        this.transferState.a().put((akv) cookieId.getMinecraftLocation(), data);
    }
    ...
    public Map<ResourceLocation, byte[]> cookies() {
        Map<akv, byte[]> vanillaCookies = this.transferState.a();
        Map<ResourceLocation, byte[]> cookies = new HashMap<>(vanillaCookies.size());
        for (Map.Entry<akv, byte[]> entry : vanillaCookies.entrySet()) {
