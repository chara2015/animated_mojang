...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_20_6/client/world/LabyBiomeBlender.class */
public class LabyBiomeBlender implements dbd {
    ...
    public LabyBiomeBlender setLevel(dca level) {
        this.level = level;
        return this;
    }
    ...
    public float a(@NotNull je direction, boolean shade) {
    ...
    @Nullable
    public dpj c_(@NotNull iz blockPos) {
        return this.level.c_(blockPos);
    }

    @NotNull
    public dse a_(@NotNull iz blockPos) {
        return this.level.a_(blockPos);
    }

    @NotNull
    public enx b_(@NotNull iz blockPos) {
    ...
    public int a(@NotNull iz blockPos, @NotNull dbl resolver) {
        int x = blockPos.u();
        int y = blockPos.v();
        int z = blockPos.w();
        int[] rgb = new int[3];
    ...
    private int processZAxis(int x, int y, int z, a currentPos, dbl resolver, int total, int[] rgb, int xIndex) {
    ...
    private int blendColors(a currentPos, dbl resolver, int total, int[] rgb) {
        ji<dcz> holder = this.level.t(currentPos);
        dcz biome = holder == null ? null : (dcz) holder.a();
        if (biome != null) {
