...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_11/client/resources/texture/VersionedTextureRepository.class */
@Singleton
...
public class VersionedTextureRepository extends AbstractTextureRepository implements TextureRepository {
    ...
    public void onDevTick() {
        ...
        for (ResourceLocation resourceLocation : this.labyAPI.renderPipeline().resources().resourceLocationFactory().getCachedResourceLocations().values()) {
            if (resourceLocation instanceof PathResourceLocation) {
                ...
                amo minecraftLocation = (amo) resourceLocation.getMinecraftLocation();
    ...
    public void register(ResourceLocation location, Texture texture, TextureRepository.TextureRegistrationCallback callback) {
        ikz minecraftTexture = MinecraftUtil.toMinecraft(texture);
    ...
    public void preloadTexture(ResourceLocation location) {
        registerTexture(location, new ili((amo) location.getMinecraftLocation()));
    }
    ...
    public Texture getTexture(ResourceLocation location) {
        ...
        Texture textureB = getTextureManager().b((amo) location.getMinecraftLocation());
    ...
    private void release(ResourceLocation location, Runnable releaseHandler) {
        getTextureManager().c((amo) location.getMinecraftLocation());
    ...
    private void registerTexture(@NotNull ResourceLocation location, ikz texture) {
        ilr textureManager = getTextureManager();
        if (texture instanceof ilh) {
            ilh reloadableTexture = (ilh) texture;
            textureManager.a((amo) location.getMinecraftLocation(), reloadableTexture);
        } else {
            textureManager.a((amo) location.getMinecraftLocation(), texture);
        }
    }

    private void registerTexture(ResourceLocation location, ikz texture, TextureRepository.TextureRegistrationCallback callback) {
    ...
    private ilr getTextureManager() {
        if (this.textureManager == null) {
            this.textureManager = gfj.V().af();
        }
        return this.textureManager;
    }

    private Texture createTexture(ikz texture) {
