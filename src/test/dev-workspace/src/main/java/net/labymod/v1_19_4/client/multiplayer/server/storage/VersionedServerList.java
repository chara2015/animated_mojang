package net.labymod.v1_19_4.client.multiplayer.server.storage;
...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_19_4/client/multiplayer/server/storage/VersionedServerList.class */
public class VersionedServerList extends DefaultServerList {
    ...
    public void load() {
        try {
            ...
            re root = ro.b(new File(this.gameDirectory, "servers.dat"));
            if (root == null) {
                return;
            }
            rk list = root.c("servers", 10);
    ...
    private re serverDataToTag(StorageServerData data) {
        re tag = new re();
    ...
    private StorageServerData tagToServerData(re tag) {
        String strF_;
        ...
        if (tag.b("labyModMetadata", 10)) {
            re metaTag = tag.p("labyModMetadata");
            for (String key : metaTag.e()) {
                rb rbVarC = metaTag.c(key);
                if (rbVarC instanceof rv) {
                    strF_ = rbVarC.f_();
                } else if (rbVarC instanceof rb) {
                    rb byteArrayTag = rbVarC;
                    strF_ = new String(byteArrayTag.e(), StandardCharsets.UTF_8);
                }
                String value = strF_;
