...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_10/client/gui/screen/VersionedScreenWrapper.class */
public class VersionedScreenWrapper implements ScreenWrapper {
    private final gmj screen;
    private final gdd graphics;
    private Metadata metadata;
    private boolean initialized;
    private long lastClick;
    private int lastClickMouseButton;

    public VersionedScreenWrapper(gmj screen) {
        if (screen == null) {
            throw new NullPointerException("screen is null");
        }
        fzz minecraft = fzz.W();
        this.graphics = new gdd(minecraft, GameRendererAccessor.self(fzz.W().i).labyMod$getGuiRenderState());
        this.screen = screen;
    ...
    public void initialize(Parent parent) {
        if (parent != null) {
            this.screen.b(fzz.W(), (int) parent.bounds().getWidth(BoundsType.INNER), (int) parent.bounds().getHeight(BoundsType.INNER));
        } else {
            Window window = Laby.labyAPI().minecraft().minecraftWindow();
            this.screen.b(fzz.W(), window.getScaledWidth(), window.getScaledHeight());
        }
    ...
    public void resize(int width, int height) {
        this.screen.b(fzz.W(), width, height);
        this.initialized = true;
    }

    @Override // net.labymod.api.client.gui.screen.ScreenInstance
    public void render(ScreenContext context) {
        this.graphics.setPose((Matrix3x2fStack) context.stack().getProvider().getPoseStack());
    ...
    public boolean keyPressed(Key key, InputType type) {
        return this.screen.a(new gth(key.getId(), DefaultKeyMapper.getGlfwScancode(), DefaultKeyMapper.getGlfwModifiers()));
    }

    @Override // net.labymod.api.client.gui.KeyboardUser
    public boolean keyReleased(Key key, InputType type) {
        return this.screen.b(new gth(key.getId(), DefaultKeyMapper.getGlfwScancode(), DefaultKeyMapper.getGlfwModifiers()));
    }

    @Override // net.labymod.api.client.gui.KeyboardUser
    public boolean charTyped(Key key, char character) {
        return this.screen.a(new gte(character, key.getId()));
    }
    ...
    public boolean mouseReleased(MutableMouse mouse, MouseButton mouseButton) {
        return this.screen.a(new gti(mouse.getXDouble(), mouse.getYDouble(), new gtj(mouseButton.getId(), DefaultKeyMapper.getGlfwMouseModifiers())));
    }
    ...
    public boolean mouseClicked(MutableMouse mouse, MouseButton mouseButton) {
        ...
        if (this.screen.a(new gti(mouse.getXDouble(), mouse.getYDouble(), new gtj(mouseButton.getId(), DefaultKeyMapper.getGlfwMouseModifiers())), doubleClick)) {
    ...
    public boolean mouseDragged(MutableMouse mouse, MouseButton button, double deltaX, double deltaY) {
        return this.screen.a(new gti(mouse.getXDouble(), mouse.getYDouble(), new gtj(button.getId(), DefaultKeyMapper.getGlfwMouseModifiers())), deltaX, deltaY);
    }
