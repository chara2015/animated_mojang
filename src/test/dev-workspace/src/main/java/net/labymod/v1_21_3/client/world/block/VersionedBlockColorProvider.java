package net.labymod.v1_21_3.client.world.block;
...
import net.labymod.v1_21_3.client.world.LabyBiomeBlender;

/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_3/client/world/block/VersionedBlockColorProvider.class */
@Singleton
...
public class VersionedBlockColorProvider implements BlockColorProvider {
    ...
    public int getColor(BlockState state) {
        alz blockId = ma.e.b(state.block());
    ...
    public int getColorMultiplier(BlockState state, int minX, int minZ, int maxX, int maxZ) {
        ...
        fni blockColors = fmg.Q().aw();
        return blockColors.a((dxv) state, this.biomeBlender.setLevel(dhiVar).setArea(minX, minZ, maxX, maxZ), toBlockPos(state.position()), 0);
    }

    private jh toBlockPos(IntVector3 vec) {
        return new jh(vec.getX(), vec.getY(), vec.getZ());
    }
}
