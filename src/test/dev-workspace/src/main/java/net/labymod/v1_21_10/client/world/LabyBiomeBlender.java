...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_10/client/world/LabyBiomeBlender.class */
public class LabyBiomeBlender implements dqr {
    ...
    public LabyBiomeBlender setLevel(drq level) {
        this.level = level;
        return this;
    }
    ...
    public float a(@NotNull jg direction, boolean shade) {
        return this.level.a(direction, shade);
    }

    @NotNull
    public ffp E_() {
        return this.level.E_();
    }

    @Nullable
    public egg c_(@NotNull ja blockPos) {
        return this.level.c_(blockPos);
    }

    @NotNull
    public ejm a_(@NotNull ja blockPos) {
        return this.level.a_(blockPos);
    }

    @NotNull
    public fga b_(@NotNull ja blockPos) {
    ...
    public int a(@NotNull ja blockPos, @NotNull dra resolver) {
        int x = blockPos.u();
        int y = blockPos.v();
        int z = blockPos.w();
        int[] rgb = new int[3];
    ...
    private int processZAxis(int x, int y, int z, a currentPos, dra resolver, int total, int[] rgb, int xIndex) {
    ...
    private int blendColors(a currentPos, dra resolver, int total, int[] rgb) {
        jk<dst> holder = this.level.v(currentPos);
        dst biome = holder == null ? null : (dst) holder.a();
        if (biome != null) {
