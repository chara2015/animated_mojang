...
import net.labymod.v1_20_1.laby3d.pipeline.LabyShaderInstance;
import org.spongepowered.asm.mixin.Final;
import org.spongepowered.asm.mixin.Mixin;
import org.spongepowered.asm.mixin.Shadow;
import org.spongepowered.asm.mixin.injection.At;

/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_20_1/mixins/client/renderer/MixinShaderInstance_Laby3DApi.class */
@Mixin({fki.class})
public class MixinShaderInstance_Laby3DApi {
    ...
    private acq labyMod$rewriteId(String id, Operation<acq> original) {
        if (this instanceof LabyShaderInstance) {
            return (acq) LabyShaderInstance.rewriteId(id, this.F).getMinecraftLocation();
        }
        return (acq) original.call(new Object[]{id});
    }

    @WrapOperation(method = {"getOrCreate"}, at = {@At(value = "NEW", target = "(Ljava/lang/String;)Lnet/minecraft/resources/ResourceLocation;")})
    private static acq labyMod$rewriteProgramId(String id, Operation<acq> original, @Local(argsOnly = true) String name) {
        if (name.contains(String.valueOf(':'))) {
            return (acq) LabyShaderInstance.rewriteId(id, name).getMinecraftLocation();
        }
        return (acq) original.call(new Object[]{id});
    }
