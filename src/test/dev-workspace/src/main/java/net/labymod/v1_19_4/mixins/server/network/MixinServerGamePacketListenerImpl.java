...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_19_4/mixins/server/network/MixinServerGamePacketListenerImpl.class */
@Mixin({aji.class})
public class MixinServerGamePacketListenerImpl {
    ...
    private void labyMod$fireDisconnectEventPre(tj reason, InsertInfo ci) {
        GameProfile profile = this.b.fI();
        Laby.fireEvent(new IntegratedServerPlayerDisconnectEvent(Phase.PRE, profile));
    }

    @Insert(method = {"onDisconnect"}, at = @At("TAIL"))
    private void labyMod$fireDisconnectEventPost(tj reason, InsertInfo ci) {
        GameProfile profile = this.b.fI();
