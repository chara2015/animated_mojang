...
import net.labymod.v1_21_11.client.multiplayer.server.storage.VersionedServerList;
import net.labymod.v1_21_11.client.util.MinecraftUtil;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;

/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_11/client/multiplayer/server/VersionedServerController.class */
@Singleton
...
public class VersionedServerController extends DefaultAbstractServerController {
    ...
    public ServerData createServerData(Object mojangServerData) {
        ...
        if (!(mojangServerData instanceof hit)) {
            throw new IllegalArgumentException(String.format(Locale.ROOT, "%s is not an instance of %s!", mojangServerData.getClass().getName(), hit.class.getName()));
        }
        hit serverData = (hit) mojangServerData;
    ...
    private void sendCustomPayload(@NotNull hig connection, @NotNull ResourceLocation identifier, byte[] payload) {
        connection.b(new aby(new LabyModCustomPacketPayload(identifier, payload)));
    }

    private Component fromMinecraftComponent(yh component) {
        if (component == null) {
    ...
    private List<Component> fromMinecraftComponentCollection(@Nullable List<yh> list) {
        if (list == null || list.isEmpty()) {
            return new ArrayList();
        }
        List<Component> result = new ArrayList<>(list.size());
        for (yh component : list) {
    ...
    public void leaveServer() {
        ...
        boolean singleplayer = minecraft.Y();
        hit currentServer = minecraft.X();
        ...
        if (singleplayer) {
            ...
            display = new fzu(gsdVar);
        } else {
            display = new gvp(gsdVar);
        }
        minecraft.a(display);
    }
    ...
    public void joinServer(@NotNull JoinServerRequest request) {
        ...
        if (requestedScreen instanceof gsb) {
            gsb screenInstance = (gsb) requestedScreen;
            screen = screenInstance;
        }
        grb.a(screen, gfj.V(), hjw.a(serverData.b), serverData, false, (hix) null);
        if (gfj.V().x instanceof grj) {
    ...
    protected String getDefaultServerName() {
        return imu.a("selectServer.defaultName", new Object[0]);
    }
    ...
    private gsb createManageServer(StorageServerData data, String translationKey, Consumer<StorageServerData> callback) {
        gsb prevScreen = gfj.V().x;
        hit mcData = new hit(data.getName(), data.address().toString(), MinecraftUtil.toMinecraft(data.type()));
        mcData.a(a.values()[data.resourcePackStatus().ordinal()]);
        return new grs(prevScreen, yh.c(translationKey), addServer -> {
            if (addServer) {
