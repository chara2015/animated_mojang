...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_18_2/client/resources/pack/VersionedResourcePackRepository.class */
@Singleton
...
public class VersionedResourcePackRepository extends AbstractResourcePackRepository<afl> {
    ...
    public void registerSilentPack(ResourcePack pack) {
        ...
        SilentReloadableResourceManager silentReloadableResourceManagerN = dyr.D().N();
        if (silentReloadableResourceManagerN instanceof afy) {
            ((afy) silentReloadableResourceManagerN).loadSilent(pack);
    ...
    public void onRebuildSelected(List<afl> selected) {
        this.selectedPacks.clear();
        setHasServerPackSelected(false);
        for (afl pack : selected) {
            this.selectedPacks.add(pack.e());
            checkServerPack(pack);
        }
    }

    private void checkServerPack(afl pack) {
        afa resources = pack.d();
        try {
    ...
    public PackSelectionModel createSelectionModel(@NotNull Consumer<PackSelectionModel> onListChange) {
        PackSelectionModel model = new egr(() -> {
        }, pack -> {
            return null;
        }, dyr.D().O(), packRepository -> {
