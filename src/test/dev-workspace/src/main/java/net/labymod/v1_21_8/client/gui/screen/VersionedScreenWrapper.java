...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_8/client/gui/screen/VersionedScreenWrapper.class */
public class VersionedScreenWrapper implements ScreenWrapper {
    private final get screen;
    private final fxb graphics;
    private Metadata metadata;
    private boolean initialized;

    public VersionedScreenWrapper(get screen) {
        if (screen == null) {
            throw new NullPointerException("screen is null");
        }
        fue minecraft = fue.R();
        this.graphics = new fxb(minecraft, GameRendererAccessor.self(fue.R().j).labyMod$getGuiRenderState());
        this.screen = screen;
    ...
    public void initialize(Parent parent) {
        if (parent != null) {
            this.screen.b(fue.R(), (int) parent.bounds().getWidth(BoundsType.INNER), (int) parent.bounds().getHeight(BoundsType.INNER));
        } else {
            Window window = Laby.labyAPI().minecraft().minecraftWindow();
            this.screen.b(fue.R(), window.getScaledWidth(), window.getScaledHeight());
        }
    ...
    public void resize(int width, int height) {
        this.screen.b(fue.R(), width, height);
        this.initialized = true;
    }
