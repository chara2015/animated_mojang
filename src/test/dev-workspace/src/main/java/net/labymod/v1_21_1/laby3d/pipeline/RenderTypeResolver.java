...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_1/laby3d/pipeline/RenderTypeResolver.class */
public final class RenderTypeResolver {
    ...
    private static final Function<LevelMaterial, gfh> DYNAMIC_TYPES = MEMOIZE.memoize(levelMaterial -> {
        MinecraftRenderState renderState = LINKER.get(levelMaterial.renderState());
        return gfh.a(getRenderTypeName(levelMaterial.id()), renderState.getVertexFormat(), renderState.getDrawMode(), 65536, renderState.affectsCrumbling(), renderState.sortOnUpload(), renderState.create(levelMaterial, false));
    });
    ...
    public gfh resolve(Material material) {
