...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_16_5/client/gui/screen/VersionedScreenWrapper.class */
public class VersionedScreenWrapper implements ScreenWrapper {
    private final dot screen;
    private Metadata metadata;
    private boolean initialized;

    public VersionedScreenWrapper(dot screen) {
        if (screen == null) {
    ...
    public void initialize(Parent parent) {
        if (parent != null) {
            this.screen.b(djz.C(), (int) parent.bounds().getWidth(BoundsType.INNER), (int) parent.bounds().getHeight(BoundsType.INNER));
        } else {
            Window window = Laby.labyAPI().minecraft().minecraftWindow();
            this.screen.b(djz.C(), window.getScaledWidth(), window.getScaledHeight());
        }
    ...
    public void resize(int width, int height) {
        this.screen.b(djz.C(), width, height);
        this.initialized = true;
    }
    ...
    public void render(ScreenContext context) {
        ...
        dfm poseStack = (dfm) context.stack().getProvider().getPoseStack();
