...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_10/client/gfx/pipeline/renderer/text/VanillaGlyphUtil.class */
public final class VanillaGlyphUtil {
    ...
    private static final BiFunction<a, hgk, Material> MATERIAL = MEMOIZE.memoize((displayMode, renderType) -> {
        ResourceLocation location = Textures.WHITE;
        if (renderType instanceof a) {
            a compositeRenderType = (a) renderType;
            UnmodifiableIterator it = compositeRenderType.C.d.iterator();
            while (true) {
                if (!it.hasNext()) {
                    break;
                }
        ...
        return LevelMaterial.builder(FontRenderStates.resolve(select(displayMode), COLORED.apply(renderType.I()).booleanValue())).setTexture(0, location).useLightmap().build();
    });

    /* JADX INFO: renamed from: net.labymod.v1_21_10.client.gfx.pipeline.renderer.text.VanillaGlyphUtil$1, reason: invalid class name */
    /* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_10/client/gfx/pipeline/renderer/text/VanillaGlyphUtil$1.class */
    ...
    public static Material material(a displayMode, hgk renderType) {
        return MATERIAL.apply(displayMode, renderType);
    }

    public static Material material(gih renderable, int flags) {
        a displayMode = select(flags);
        hgk renderType = renderable.a(displayMode);
