...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21/mixins/client/gui/components/MixinGuiEventListener.class */
@Mixin({fki.class})
public interface MixinGuiEventListener {
    ...
    @Overwrite
    default boolean b(double mouseX, double mouseY, int button) {
        return handleConvertedWidget((converter, widget) -> {
            return Boolean.valueOf(minecraft().updateMouse(mouseX, mouseY, mouse -> {
                MouseButton mouseButton = DefaultKeyMapper.pressMouse(button);
                if (mouseButton == null) {
    ...
    @Overwrite
    default boolean c(int keyCode, int param1, int param2) {
        return handleConvertedWidget((converter, widget) -> {
