...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_17_1/client/gui/screen/VersionedScreenWrapper.class */
public class VersionedScreenWrapper implements ScreenWrapper {
    private final eaq screen;
    private Metadata metadata;
    private boolean initialized;

    public VersionedScreenWrapper(eaq screen) {
        if (screen == null) {
    ...
    public void initialize(Parent parent) {
        if (parent != null) {
            this.screen.b(dvp.C(), (int) parent.bounds().getWidth(BoundsType.INNER), (int) parent.bounds().getHeight(BoundsType.INNER));
        } else {
            Window window = Laby.labyAPI().minecraft().minecraftWindow();
            this.screen.b(dvp.C(), window.getScaledWidth(), window.getScaledHeight());
        }
    ...
    public void resize(int width, int height) {
        this.screen.b(dvp.C(), width, height);
        this.initialized = true;
    }
    ...
    public void render(ScreenContext context) {
        ...
        dql poseStack = (dql) context.stack().getProvider().getPoseStack();
