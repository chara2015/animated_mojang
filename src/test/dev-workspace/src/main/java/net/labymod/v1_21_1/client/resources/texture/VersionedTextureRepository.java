...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_1/client/resources/texture/VersionedTextureRepository.class */
@Singleton
...
public class VersionedTextureRepository extends AbstractTextureRepository implements TextureRepository {
    ...
    public void onDevTick() {
        ...
        for (ResourceLocation resourceLocation : this.labyAPI.renderPipeline().resources().resourceLocationFactory().getCachedResourceLocations().values()) {
            if (resourceLocation instanceof PathResourceLocation) {
                ...
                akr minecraftLocation = (akr) resourceLocation.getMinecraftLocation();
    ...
    public void register(ResourceLocation location, Texture texture, TextureRepository.TextureRegistrationCallback callback) {
        gpw minecraftTexture = MinecraftUtil.toMinecraft(texture);
    ...
    public void preloadTexture(ResourceLocation location) {
        registerTexture(location, new gqe((akr) location.getMinecraftLocation()));
    }
    ...
    public Texture getTexture(ResourceLocation location) {
        ...
        Texture textureB = getTextureManager().b((akr) location.getMinecraftLocation());
    ...
    private void release(ResourceLocation location, Runnable releaseHandler) {
        getTextureManager().c((akr) location.getMinecraftLocation());
    ...
    private void registerTexture(@NotNull ResourceLocation location, gpw texture) {
        getTextureManager().a((akr) location.getMinecraftLocation(), texture);
    }

    private void registerTexture(@NotNull ResourceLocation location, gpw texture, TextureRepository.TextureRegistrationCallback callback) {
    ...
    private Texture createTexture(gpw texture) {
