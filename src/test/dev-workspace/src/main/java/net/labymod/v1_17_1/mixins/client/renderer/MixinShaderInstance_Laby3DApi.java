...
import net.labymod.v1_17_1.laby3d.pipeline.LabyShaderInstance;
import org.spongepowered.asm.mixin.Final;
import org.spongepowered.asm.mixin.Mixin;
import org.spongepowered.asm.mixin.Shadow;
import org.spongepowered.asm.mixin.injection.At;

/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_17_1/mixins/client/renderer/MixinShaderInstance_Laby3DApi.class */
@Mixin({ent.class})
public class MixinShaderInstance_Laby3DApi {
    ...
    private ww labyMod$rewriteId(String id, Operation<ww> original) {
        if (this instanceof LabyShaderInstance) {
            return (ww) LabyShaderInstance.rewriteId(id, this.B).getMinecraftLocation();
        }
        return (ww) original.call(new Object[]{id});
    }

    @WrapOperation(method = {"getOrCreate"}, at = {@At(value = "NEW", target = "(Ljava/lang/String;)Lnet/minecraft/resources/ResourceLocation;")})
    private static ww labyMod$rewriteProgramId(String id, Operation<ww> original, @Local(argsOnly = true) String name) {
        if (name.contains(String.valueOf(':'))) {
            return (ww) LabyShaderInstance.rewriteId(id, name).getMinecraftLocation();
        }
        return (ww) original.call(new Object[]{id});
    }
