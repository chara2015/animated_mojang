...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_18_2/client/world/block/properties/VersionedBlockPropertyHolder.class */
public class VersionedBlockPropertyHolder extends AbstractBlockPropertyHolder {
    private final cox<?, ?> holder;

    public VersionedBlockPropertyHolder(cox<?, ?> holder) {
        this.holder = holder;
    }
    ...
    protected <T extends Comparable<T>> T getPropertyValue(BlockProperty blockProperty) {
        cpy cpyVar = (cpy) blockProperty;
        if (!this.holder.b(cpyVar)) {
            return null;
        }
        Enum r0 = (T) this.holder.c(cpyVar);
    ...
    public boolean hasProperty(BlockProperty property) {
        return this.holder.b((cpy) property);
    }
}
