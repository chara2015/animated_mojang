package net.labymod.v1_21_10.client.entity;
...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_10/client/entity/VersionedEntityVisualizer.class */
@Singleton
...
public class VersionedEntityVisualizer implements EntityVisualizer {
    ...
    private void submitToGuiGraphics(Entity entity, float x, float y, float rotationX, float rotationZ, float scale, boolean renderName, boolean clipToBounds) {
        ...
        int x1;
        int y1;
        cuj cujVar = (cdv) entity;
        if (cujVar instanceof cug) {
            cug dragonPart = (cug) cujVar;
            cujVar = dragonPart.a;
        }
        hnw dispatcher = fzz.W().aw();
        hnx<? super cdv, ?> renderer = dispatcher.a(cujVar);
        huk renderState = renderer.b(cujVar, 1.0f);
        renderState.V = null;
        renderState.Q = 0;
        if (!renderName) {
            renderState.S = null;
        ...
        gdd graphics = MinecraftUtil.getCurrentGuiGraphics();
