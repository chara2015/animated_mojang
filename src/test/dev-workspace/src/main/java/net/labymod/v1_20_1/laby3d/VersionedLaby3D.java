package net.labymod.v1_20_1.laby3d;
...
import net.labymod.v1_20_1.client.resources.texture.NativeGameImage;
import org.jetbrains.annotations.NotNull;

/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_20_1/laby3d/VersionedLaby3D.class */
@Singleton
...
public class VersionedLaby3D extends Laby3D {
    ...
    public void writeToTexture(@NotNull DeviceTexture texture, @NotNull GameImage image) throws MatchException {
        if (image instanceof NativeGameImage) {
            try {
                ehk nativeImage = ((NativeGameImage) image).image();
    ...
    public void setupOverlayAndLightingTextures(@NotNull CommandBuffer cmd) {
        fjq gameRenderer = enn.N().j;
        cmd.bindTexture(1, TextureViewProvider.cast(gameRenderer.o()).getDeviceTextureView());
        cmd.bindTexture(2, TextureViewProvider.cast(gameRenderer.n()).getDeviceTextureView());
    }

    @Override // net.labymod.api.laby3d.Laby3D
    @NotNull
    public DeviceTextureView overlayTextureView() {
        fjq renderer = enn.N().j;
        return TextureViewProvider.cast(renderer.o()).getDeviceTextureView();
    }
