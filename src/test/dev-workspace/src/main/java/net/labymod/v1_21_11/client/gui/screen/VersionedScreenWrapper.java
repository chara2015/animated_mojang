...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_11/client/gui/screen/VersionedScreenWrapper.class */
public class VersionedScreenWrapper implements ScreenWrapper {
    private final gsb screen;
    private Metadata metadata;
    private boolean initialized;
    private long lastClick;
    private int lastClickMouseButton;

    public VersionedScreenWrapper(gsb screen) {
        if (screen == null) {
    ...
    public void initialize(Parent parent) {
        if (parent != null) {
            this.screen.b((int) parent.bounds().getWidth(BoundsType.INNER), (int) parent.bounds().getHeight(BoundsType.INNER));
        } else {
            Window window = Laby.labyAPI().minecraft().minecraftWindow();
            this.screen.b(window.getScaledWidth(), window.getScaledHeight());
        }
    ...
    public void render(ScreenContext context) {
        ...
        this.screen.c(currentGuiGraphics, mouse.getX(), mouse.getY(), context.getTickDelta());
    }
    ...
    public boolean keyPressed(Key key, InputType type) {
        return this.screen.a(new gzb(key.getId(), DefaultKeyMapper.getGlfwScancode(), DefaultKeyMapper.getGlfwModifiers()));
    }

    @Override // net.labymod.api.client.gui.KeyboardUser
    public boolean keyReleased(Key key, InputType type) {
        return this.screen.b(new gzb(key.getId(), DefaultKeyMapper.getGlfwScancode(), DefaultKeyMapper.getGlfwModifiers()));
    }

    @Override // net.labymod.api.client.gui.KeyboardUser
    public boolean charTyped(Key key, char character) {
        return this.screen.a(new gyy(character, key.getId()));
    }
    ...
    public boolean mouseReleased(MutableMouse mouse, MouseButton mouseButton) {
        return this.screen.b(new gzc(mouse.getXDouble(), mouse.getYDouble(), new gzd(mouseButton.getId(), DefaultKeyMapper.getGlfwMouseModifiers())));
    }
    ...
    public boolean mouseClicked(MutableMouse mouse, MouseButton mouseButton) {
        ...
        if (this.screen.a(new gzc(mouse.getXDouble(), mouse.getYDouble(), new gzd(mouseButton.getId(), DefaultKeyMapper.getGlfwMouseModifiers())), doubleClick)) {
    ...
    public boolean mouseDragged(MutableMouse mouse, MouseButton button, double deltaX, double deltaY) {
        return this.screen.a(new gzc(mouse.getXDouble(), mouse.getYDouble(), new gzd(button.getId(), DefaultKeyMapper.getGlfwMouseModifiers())), deltaX, deltaY);
    }
