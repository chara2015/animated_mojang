package net.labymod.v1_20_6.mixins.client.renderer.renderer.atlas;

import net.labymod.v1_20_6.client.gfx.pipeline.texture.atlas.TextureAtlasHolderAccessor;
import org.spongepowered.asm.mixin.Final;
import org.spongepowered.asm.mixin.Mixin;
import org.spongepowered.asm.mixin.Shadow;

/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_20_6/mixins/client/renderer/renderer/atlas/MixinTextureAtlasHolder.class */
@Mixin({gqe.class})
public class MixinTextureAtlasHolder implements TextureAtlasHolderAccessor {

    @Shadow
    @Final
    private gpa a;

    @Override // net.labymod.v1_20_6.client.gfx.pipeline.texture.atlas.TextureAtlasHolderAccessor
    public gpa getTextureAtlas() {
        return this.a;
    }
}
