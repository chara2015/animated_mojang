...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_8_9/client/gfx/pipeline/util/VersionedScreenUtil.class */
public final class VersionedScreenUtil {
    private static int lastMouseX = -1;
    private static int lastMouseY = -1;

    @Deprecated
    public static void drawScreen(axu screen, int mouseX, int mouseY, float tickDelta) {
        handleMouseDragged();
        ...
        MinecraftUtil.obtainScreenContextFromStack(VersionedStackProvider.DEFAULT_STACK, mouseX, mouseY, tickDelta, screenContext -> {
            ScreenUtil.wrapRender(screenContext, () -> {
                screen.a(mouseX, mouseY, tickDelta);
            });
        });
        screenCustomFontStack.pop(screen);
    }

    public static void drawScreen(Operation<Void> original, axu screen, int mouseX, int mouseY, float tickDelta) {
        handleMouseDragged();
        ScreenCustomFontStack screenCustomFontStack = Laby.references().screenCustomFontStack();
        screenCustomFontStack.push(screen);
        MinecraftUtil.obtainScreenContextFromStack(VersionedStackProvider.DEFAULT_STACK, mouseX, mouseY, tickDelta, screenContext -> {
    ...
    private static void handleMouseDragged() {
        ...
        VersionedMinecraft versionedMinecraftA = ave.A();
        MouseButton mouseButton = versionedMinecraftA.getCurrentEventButton();
        if (lastMouseX != -1 && lastMouseY != -1 && mouseButton != null) {
            Window window = Laby.labyAPI().minecraft().minecraftWindow();
            double widthFactor = ((double) window.getScaledWidth()) / ((double) ((ave) versionedMinecraftA).d);
            double heightFactor = ((double) window.getScaledHeight()) / ((double) ((ave) versionedMinecraftA).e);
            ...
            Laby.labyAPI().minecraft().updateMouse(((double) rawMouseX) * widthFactor, ((double) window.getScaledHeight()) - (((double) rawMouseY) * heightFactor), mouse -> {
                ((VersionedMinecraft) versionedMinecraftA).handleMouseDragged(mouse, mouseButton, deltaX, deltaY);
            });
        }
