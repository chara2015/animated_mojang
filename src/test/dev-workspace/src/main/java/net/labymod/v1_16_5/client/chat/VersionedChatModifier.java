...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_16_5/client/chat/VersionedChatModifier.class */
@Singleton
...
public class VersionedChatModifier implements InternalChatModifier {
    ...
    public void deleteMessage(ChatMessage message) {
        VersionedChatComponent accessor = djz.C().j.c();
    ...
    public void editMessage(ChatMessage message, Component component) {
        ...
        dju<nr> prevMessage = accessor.getMessages().get(index);
        VersionedChatMessageComponent versionedChatMessageComponent = new VersionedChatMessageComponent(message, (nr) this.componentMapper.toMinecraftComponent(component));
        accessor.getMessages().set(index, new dju<>(prevMessage.b(), versionedChatMessageComponent, prevMessage.c()));
        Predicate<dju<afa>> predicate = formattedFilter(message);
        int first = -1;
        int i = 0;
        while (true) {
    ...
    private Predicate<dju<nr>> messageFilter(ChatMessage message) {
        return m -> {
    ...
    private Predicate<dju<afa>> formattedFilter(ChatMessage message) {
        return m -> {
