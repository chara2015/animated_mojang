package net.labymod.v1_19_4.client.gui.screen;
...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_19_4/client/gui/screen/VersionedScreenWrapper.class */
public class VersionedScreenWrapper implements ScreenWrapper {
    private final etd screen;
    private Metadata metadata;
    private boolean initialized;

    public VersionedScreenWrapper(etd screen) {
        if (screen == null) {
    ...
    public void initialize(Parent parent) {
        if (parent != null) {
            this.screen.b(emh.N(), (int) parent.bounds().getWidth(BoundsType.INNER), (int) parent.bounds().getHeight(BoundsType.INNER));
        } else {
            Window window = Laby.labyAPI().minecraft().minecraftWindow();
            this.screen.b(emh.N(), window.getScaledWidth(), window.getScaledHeight());
        }
    ...
    public void resize(int width, int height) {
        this.screen.b(emh.N(), width, height);
        this.initialized = true;
    }
    ...
    public void render(ScreenContext context) {
        ...
        ehe poseStack = (ehe) context.stack().getProvider().getPoseStack();
        this.screen.b(poseStack, mouse.getX(), mouse.getY(), context.getTickDelta());
    }
