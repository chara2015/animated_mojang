package net.labymod.v1_18_2.client.world.block;
...
import net.labymod.v1_18_2.client.world.LabyBiomeBlender;

/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_18_2/client/world/block/VersionedBlockColorProvider.class */
@Singleton
...
public class VersionedBlockColorProvider implements BlockColorProvider {
    ...
    public int getColorMultiplier(BlockState state, int minX, int minZ, int maxX, int maxZ) {
        ...
        return blockColors.a((cov) state, this.biomeBlender.setLevel(cavVar).setArea(minX, minZ, maxX, maxZ), toBlockPos(state.position()), 0);
    }

    private gj toBlockPos(IntVector3 vec) {
        return new gj(vec.getX(), vec.getY(), vec.getZ());
    }
}
