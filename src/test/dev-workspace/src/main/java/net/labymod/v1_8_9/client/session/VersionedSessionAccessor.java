...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_8_9/client/session/VersionedSessionAccessor.class */
@Singleton
@Implements(SessionAccessor.class)
public class VersionedSessionAccessor extends DefaultAbstractSessionAccessor<avm> {
    @Inject
    ...
    public avm toGameSession(Session session) {
        Session.Type type = session.getType();
        if (type == Session.Type.MICROSOFT) {
            type = Session.Type.MOJANG;
        }
        return new avm(session.getUsername(), session.getUniqueId().toString(), session.getAccessToken(), type.toString());
    }

    @Override // net.labymod.core.client.session.DefaultAbstractSessionAccessor
    public Session fromGameSession(avm gameSession) {
        if (gameSession == null) {
            return null;
        }
