...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_11/client/world/block/VersionedBlockColorProvider.class */
@Singleton
...
public class VersionedBlockColorProvider implements BlockColorProvider {
    ...
    public int getColor(BlockState state) {
        amo blockId = mi.e.b(state.block());
    ...
    public int getColorMultiplier(BlockState state, int minX, int minZ, int maxX, int maxZ) {
        ...
        return blockColors.a((eoh) state, this.biomeBlender.setLevel(dwoVar).setArea(minX, minZ, maxX, maxZ), toBlockPos(state.position()), 0);
    }

    private is toBlockPos(IntVector3 vec) {
        return new is(vec.getX(), vec.getY(), vec.getZ());
    }
}
