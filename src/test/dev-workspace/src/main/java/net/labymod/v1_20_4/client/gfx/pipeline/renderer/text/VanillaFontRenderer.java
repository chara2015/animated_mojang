...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_20_4/client/gfx/pipeline/renderer/text/VanillaFontRenderer.class */
@Singleton
...
public class VanillaFontRenderer implements MinecraftFontRenderer {
    ...
    public void render(Matrix4f pose, Component text, float x, float y, int argb, int packedLightCoords, int backgroundArgb, int flags) {
        ews font = getFont();
        font.a((vf) this.componentMapper.toMinecraftComponent(text), x, y, argb, FontFlags.isShadow(flags), pose, getBufferSource(), getDisplayMode(flags), backgroundArgb, packedLightCoords);
    }
    ...
    public float getWidth(Component text) {
        return getFont().a((vf) this.componentMapper.toMinecraftComponent(text));
    }
    ...
    public String trimTextToWidth(String text, float maxWidth, StringStart start) {
        evu splitter = getFont().b();
        if (start == StringStart.LEFT) {
            return splitter.b(text, MathHelper.ceil(maxWidth), wc.a);
        }
        return splitter.c(text, MathHelper.ceil(maxWidth), wc.a);
    }

    @Override // net.labymod.api.client.gfx.pipeline.renderer.text.FontRenderer
    public float getWidth(int codepoint, Style style) {
        return StringSplitterAccessor.cast(getFont().b()).getWidthProvider().getWidth(codepoint, (wc) CastUtil.cast(style));
    }

    private fth getBufferSource() {
        return evi.O().aO().c();
    }
    ...
    /* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_20_4/client/gfx/pipeline/renderer/text/VanillaFontRenderer$PreparedTextBuilder.class */
    public static class PreparedTextBuilder implements aub {
        ...
        public PreparedTextBuilder(ews font, float x, float y, int color, boolean drawShadow, int backgroundColor) {
        ...
        public boolean accept(int position, @NotNull wc style, int codepoint) {
            ezz ezzVarA;
            ezv fontSet = this.accessor.labyMod$getFontSet(style.k());
            eod glyphInfo = fontSet.a(codepoint, this.accessor.labyMod$filterFishyGlyphs());
            if (style.f() && codepoint != 32) {
                ezzVarA = fontSet.a(glyphInfo);
            } else {
                ezzVarA = fontSet.a(codepoint);
            }
            ezz bakedGlyph = ezzVarA;
            boolean bold = style.b();
            we textColor = style.a();
        ...
        private void addEffect(a effect) {
            ezv fontSet = this.accessor.labyMod$getFontSet(wc.b);
            ezz bakedGlyph = fontSet.a();
            this.builder.addEffect(new VanillaRenderedEffect(bakedGlyph, effect));
        }

        private int getTextColor(@Nullable we textColor) {
            if (textColor != null) {
