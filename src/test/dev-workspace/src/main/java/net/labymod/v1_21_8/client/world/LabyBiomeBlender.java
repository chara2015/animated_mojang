...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_8/client/world/LabyBiomeBlender.class */
public class LabyBiomeBlender implements dlv {
    ...
    public LabyBiomeBlender setLevel(dmu level) {
        this.level = level;
        return this;
    }
    ...
    public float a(@NotNull jh direction, boolean shade) {
    ...
    @Nullable
    public eaz c_(@NotNull jb blockPos) {
        return this.level.c_(blockPos);
    }

    @NotNull
    public eeb a_(@NotNull jb blockPos) {
        return this.level.a_(blockPos);
    }

    @NotNull
    public fal b_(@NotNull jb blockPos) {
    ...
    public int a(@NotNull jb blockPos, @NotNull dme resolver) {
        int x = blockPos.u();
        int y = blockPos.v();
        int z = blockPos.w();
        int[] rgb = new int[3];
    ...
    private int processZAxis(int x, int y, int z, a currentPos, dme resolver, int total, int[] rgb, int xIndex) {
    ...
    private int blendColors(a currentPos, dme resolver, int total, int[] rgb) {
        jl<dnx> holder = this.level.v(currentPos);
        dnx biome = holder == null ? null : (dnx) holder.a();
        if (biome != null) {
