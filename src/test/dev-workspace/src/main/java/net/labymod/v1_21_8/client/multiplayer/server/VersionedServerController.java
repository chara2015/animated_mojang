...
import net.labymod.v1_21_8.client.multiplayer.server.storage.VersionedServerList;
import net.labymod.v1_21_8.client.util.MinecraftUtil;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;

/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_8/client/multiplayer/server/VersionedServerController.class */
@Singleton
...
public class VersionedServerController extends DefaultAbstractServerController {
    ...
    public ServerData createServerData(Object mojangServerData) {
        ...
        if (!(mojangServerData instanceof grz)) {
            throw new IllegalArgumentException(String.format(Locale.ROOT, "%s is not an instance of %s!", mojangServerData.getClass().getName(), grz.class.getName()));
        }
        grz serverData = (grz) mojangServerData;
    ...
    private void sendCustomPayload(@NotNull grl connection, @NotNull ResourceLocation identifier, byte[] payload) {
        connection.b(new aaw(new LabyModCustomPacketPayload(identifier, payload)));
    }

    private Component fromMinecraftComponent(xo component) {
        if (component == null) {
    ...
    private List<Component> fromMinecraftComponentCollection(@Nullable List<xo> list) {
        if (list == null || list.isEmpty()) {
            return new ArrayList();
        }
        List<Component> result = new ArrayList<>(list.size());
        for (xo component : list) {
    ...
    public void leaveServer() {
        ...
        boolean singleplayer = minecraft.U();
        grz currentServer = minecraft.T();
        ...
        if (singleplayer) {
            ...
            display = new fol(gevVar);
        } else {
            display = new gid(gevVar);
        }
        minecraft.a(display);
    }
    ...
    public void joinServer(@NotNull JoinServerRequest request) {
        ...
        if (requestedScreen instanceof get) {
            get screenInstance = (get) requestedScreen;
            screen = screenInstance;
        }
        gds.a(screen, fue.R(), gtc.a(serverData.b), serverData, false, (gsd) null);
        if (fue.R().z instanceof gea) {
    ...
    protected String getDefaultServerName() {
        return htb.a("selectServer.defaultName", new Object[0]);
    }

    @Override // net.labymod.api.client.network.server.ServerController
    public ScreenWrapper createEditServerScreen(StorageServerData data, Consumer<StorageServerData> callback) {
        get prevScreen = fue.R().z;
        grz mcData = new grz(data.getName(), data.address().toString(), MinecraftUtil.toMinecraft(data.type()));
        mcData.a(a.values()[data.resourcePackStatus().ordinal()]);
        geb screen = new geb(prevScreen, addServer -> {
            if (addServer) {
