...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_18_2/client/resources/pack/VersionedResourcePackScanner.class */
@Singleton
...
public class VersionedResourcePackScanner extends AbstractResourcePackScanner {
    ...
    private boolean isExternalPack(afa pack) {
        return (pack instanceof aey) || (pack instanceof aez);
    }

    private IncompatibleResourcePack scanPack(afa pack) {
    ...
    private void scanPack(afa pack, String baseDirectory, Consumer<yt> output) {
        Collection<yt> locations = pack.a(afb.a, Namespaces.MINECRAFT, baseDirectory, Integer.MAX_VALUE, s -> {
            return true;
        });
        for (yt location : locations) {
            output.accept(location);
        }
    }

    private Collection<String> collectIgnoredFiles(afa pack) {
        Set<String> ignoredFiles = new HashSet<>();
        for (String namespace : pack.a(afb.a)) {
            try {
                InputStream resource = pack.a(afb.a, new yt(namespace, "labymod/ignored_files.json"));
                try {
