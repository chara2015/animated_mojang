...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_11/client/entity/VersionedEntityVisualizer.class */
@Singleton
...
public class VersionedEntityVisualizer implements EntityVisualizer {
    ...
    private void submitToGuiGraphics(Entity entity, float x, float y, float rotationX, float rotationZ, float scale, boolean renderName, boolean clipToBounds) {
        ...
        int x1;
        int y1;
        cyc cycVar = (cgk) entity;
        if (cycVar instanceof cyd) {
            cyd dragonPart = (cyd) cycVar;
            cycVar = dragonPart.a;
        }
        hwo dispatcher = gfj.V().ay();
        hwp<? super cgk, ?> renderer = dispatcher.a(cycVar);
        idf renderState = renderer.b(cycVar, 1.0f);
        renderState.U = 0;
        if (!renderName) {
            renderState.W = null;
        }
        ...
        gir graphics = MinecraftUtil.getCurrentGuiGraphics();
