...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_10/client/resources/texture/VersionedTextureRepository.class */
@Singleton
...
public class VersionedTextureRepository extends AbstractTextureRepository implements TextureRepository {
    ...
    public void onDevTick() {
        ...
        for (ResourceLocation resourceLocation : this.labyAPI.renderPipeline().resources().resourceLocationFactory().getCachedResourceLocations().values()) {
            if (resourceLocation instanceof PathResourceLocation) {
                ...
                amj minecraftLocation = (amj) resourceLocation.getMinecraftLocation();
    ...
    public void register(ResourceLocation location, Texture texture, TextureRepository.TextureRegistrationCallback callback) {
        iby minecraftTexture = MinecraftUtil.toMinecraft(texture);
    ...
    public void preloadTexture(ResourceLocation location) {
        registerTexture(location, new icg((amj) location.getMinecraftLocation()));
    }
    ...
    public Texture getTexture(ResourceLocation location) {
        ...
        Texture textureB = getTextureManager().b((amj) location.getMinecraftLocation());
    ...
    private void release(ResourceLocation location, Runnable releaseHandler) {
        getTextureManager().c((amj) location.getMinecraftLocation());
    ...
    private void registerTexture(@NotNull ResourceLocation location, iby texture) {
        icq textureManager = getTextureManager();
        if (texture instanceof icf) {
            icf reloadableTexture = (icf) texture;
            textureManager.a((amj) location.getMinecraftLocation(), reloadableTexture);
        } else {
            textureManager.a((amj) location.getMinecraftLocation(), texture);
        }
    }

    private void registerTexture(ResourceLocation location, iby texture, TextureRepository.TextureRegistrationCallback callback) {
    ...
    private icq getTextureManager() {
        if (this.textureManager == null) {
            this.textureManager = fzz.W().ag();
        }
        return this.textureManager;
    }

    private Texture createTexture(iby texture) {
