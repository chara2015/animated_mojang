...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_11/client/session/user/VersionedUserFactory.class */
@Singleton
...
public class VersionedUserFactory implements UserFactory {
    ...
    public gfx createUser(Session session) {
        UUID newUuidObject = UUID.fromString(session.getUniqueId().toString());
        return new gfx(session.getUsername(), newUuidObject, session.getAccessToken(), Optional.empty(), Optional.empty());
    }
}
