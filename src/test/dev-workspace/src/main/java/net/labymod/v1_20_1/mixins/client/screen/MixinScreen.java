...
import net.labymod.v1_20_1.client.gui.ScreenAccessor;
...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_20_1/mixins/client/screen/MixinScreen.class */
@Mixin({euq.class})
public abstract class MixinScreen implements FileDropHandler, ScreenAccessor {
    ...
    public void fireScreenInitEvent(enn mc, int width, int height, InsertInfo ci) {
        Laby.fireEvent(new VersionedScreenInitEvent(this));
    }

    @Insert(method = {"renderBackground"}, at = @At("HEAD"), cancellable = true)
    public void renderThemeBackground(eox graphics, InsertInfo ci) {
        MutableMouse mouse = Laby.labyAPI().minecraft().mouse();
        Laby.references().renderEnvironmentContext().screenContext().runInContext(graphics.c().stack(), mouse, enn.N().aw(), context -> {
            renderThemeBackground(context, ci);
        });
    }

    @Insert(method = {"renderDirtBackground"}, at = @At("HEAD"), cancellable = true)
    public void renderThemeDirtBackground(eox graphics, InsertInfo ci) {
        MutableMouse mouse = Laby.labyAPI().minecraft().mouse();
        Laby.references().renderEnvironmentContext().screenContext().runInContext(graphics.c().stack(), mouse, enn.N().aw(), context -> {
            renderThemeBackground(context, ci);
        });
    ...
    @Insert(method = {"renderBackground"}, at = @At(value = "INVOKE", target = "Lnet/minecraft/client/gui/GuiGraphics;fillGradient(IIIIII)V", shift = At.Shift.BEFORE), cancellable = true)
    private void labyMod$disableMenuBackground(eox graphics, InsertInfo ci) {
    ...
    public void renderComponentHoverEffect(Stack stack, Style style, int x, int y) {
        ...
        currentGuiGraphics.setPose((eij) stack.getProvider().getPoseStack());
        currentGuiGraphics.a(this.i, (ts) style, x, y);
    }

    @Override // net.labymod.v1_20_1.client.gui.ScreenAccessor
    public void setMinecraft(enn minecraft) {
        this.f = minecraft;
    }
    ...
    public List<Object> getWrappedWidgets() {
        ...
        for (eqt child : this.k) {
            wrappedWidgets.add(asVersionedWidget(child));
        }
        return wrappedWidgets;
    }

    private VersionedWidget asVersionedWidget(eqt child) {
        ...
        if (child instanceof eqs) {
            eqs container = (eqs) child;
            for (eqt entry : container.i()) {
