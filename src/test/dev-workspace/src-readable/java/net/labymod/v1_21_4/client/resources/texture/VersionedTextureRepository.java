...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_4/client/resources/texture/VersionedTextureRepository.class */
@Singleton
...
public class VersionedTextureRepository extends AbstractTextureRepository implements TextureRepository {
    ...
    public void onDevTick() {
        ...
        for (ResourceLocation resourceLocation : this.labyAPI.renderPipeline().resources().resourceLocationFactory().getCachedResourceLocations().values()) {
            if (resourceLocation instanceof PathResourceLocation) {
                ...
                akv minecraftLocation = (akv) resourceLocation.getMinecraftLocation();
                ...
                if (hasResource && path.isModified()) {
                    ...
                    hee texture = textureManager.b(minecraftLocation);
                    texture.c();
    ...
    public void register(ResourceLocation location, Texture texture, TextureRepository.TextureRegistrationCallback callback) {
        hee minecraftTexture = MinecraftUtil.toMinecraft(texture);
    ...
    public void preloadTexture(ResourceLocation location) {
        registerTexture(location, new hel((akv) location.getMinecraftLocation()));
    }
    ...
    public Texture getTexture(ResourceLocation location) {
        ...
        Texture textureB = getTextureManager().b((akv) location.getMinecraftLocation());
    ...
    private void release(ResourceLocation location, Runnable releaseHandler) {
        getTextureManager().c((akv) location.getMinecraftLocation());
    ...
    private void registerTexture(@NotNull ResourceLocation location, hee texture) {
        hev textureManager = getTextureManager();
        if (texture instanceof hek) {
            hek reloadableTexture = (hek) texture;
            textureManager.a((akv) location.getMinecraftLocation(), reloadableTexture);
        } else {
            textureManager.a((akv) location.getMinecraftLocation(), texture);
        }
    }

    private void registerTexture(ResourceLocation location, hee texture, TextureRepository.TextureRegistrationCallback callback) {
    ...
    private hev getTextureManager() {
        if (this.textureManager == null) {
            this.textureManager = flk.Q().aa();
        }
        return this.textureManager;
    }

    private Texture createTexture(hee texture) {
