...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_4/client/resources/pack/ModifiedPackResources.class */
public class ModifiedPackResources implements atc {
    ...
    public ModifiedPackResources(ResourcePack resourcePack) {
        ...
        this.packLocationInfo = new atb(resourcePack.getName(), wp.b(resourcePack.getName()), aub.c, Optional.empty());
    }
    ...
    public auh<InputStream> a(ate type, akv location) {
        boolean isServerData = type == ate.b;
    ...
    public void a(@NotNull ate packType, @NotNull String namespace, @NotNull String path, @NotNull a output) {
        this.resourcePack.listResources(PackTypes.select(packType == ate.b), namespace, path, (location, inputStreamSupplier) -> {
            akv akvVar = (akv) location.getMinecraftLocation();
            Objects.requireNonNull(inputStreamSupplier);
            output.accept(akvVar, inputStreamSupplier::get);
        });
    }

    public Set<String> a(ate type) {
        return this.resourcePack.getNamespaces(PackTypes.select(type == ate.b));
    }
    ...
    public <T> T a(@NotNull atp<T> atpVar) throws IOException {
        ...
        try {
            T t = (T) ast.a(atpVar, byteArrayInputStream);
