...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21/client/session/user/VersionedUserFactory.class */
@Singleton
...
public class VersionedUserFactory implements UserFactory {
    ...
    public fhb createUser(Session session) {
        UUID newUuidObject = UUID.fromString(session.getUniqueId().toString());
        return new fhb(session.getUsername(), newUuidObject, session.getAccessToken(), Optional.empty(), Optional.empty(), session.isPremium() ? a.c : a.a);
    }
}
