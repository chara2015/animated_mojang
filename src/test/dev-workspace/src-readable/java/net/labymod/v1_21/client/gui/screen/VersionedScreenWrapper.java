package net.labymod.v1_21.client.gui.screen;
...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21/client/gui/screen/VersionedScreenWrapper.class */
public class VersionedScreenWrapper implements ScreenWrapper {
    private final fod screen;
    private final fhz graphics;
    private Metadata metadata;
    private boolean initialized;

    public VersionedScreenWrapper(fod screen) {
        if (screen == null) {
            throw new NullPointerException("screen is null");
        }
        fgo minecraft = fgo.Q();
        this.graphics = new fhz(minecraft, minecraft.aO().c());
        this.screen = screen;
    ...
    public void initialize(Parent parent) {
        if (parent != null) {
            this.screen.b(fgo.Q(), (int) parent.bounds().getWidth(BoundsType.INNER), (int) parent.bounds().getHeight(BoundsType.INNER));
        } else {
            Window window = Laby.labyAPI().minecraft().minecraftWindow();
            this.screen.b(fgo.Q(), window.getScaledWidth(), window.getScaledHeight());
        }
    ...
    public void resize(int width, int height) {
        this.screen.b(fgo.Q(), width, height);
        this.initialized = true;
    }

    @Override // net.labymod.api.client.gui.screen.ScreenInstance
    public void render(ScreenContext context) {
        this.graphics.setPose((fbi) context.stack().getProvider().getPoseStack());
