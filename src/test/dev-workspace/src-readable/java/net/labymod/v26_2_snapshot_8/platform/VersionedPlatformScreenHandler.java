...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v26_2_snapshot_8/platform/VersionedPlatformScreenHandler.class */
public class VersionedPlatformScreenHandler extends PlatformScreenHandler<Screen> {
    ...
    public void onInitialize() {
        ...
        register(NamedScreen.OPEN_LAN_WORLD, MultiplayerOptionsScreen.class);
        ...
        register(NamedScreen.FRIENDS, FriendsOverlayScreen.class);
        ...
        registerFactory(NamedScreen.OPTIONS, () -> {
            Screen prevScreen = Minecraft.getInstance().gui.screen();
            if (prevScreen instanceof GenericMessageScreen) {
                prevScreen = null;
            }
            return new OptionsScreen(prevScreen, Minecraft.getInstance().options, Minecraft.getInstance().level != null);
        });
        ...
        registerFactory(NamedScreen.LANGUAGE_SELECTION, () -> {
            return new LanguageSelectScreen(Minecraft.getInstance().gui.screen(), Minecraft.getInstance().options, Minecraft.getInstance().getLanguageManager());
        });
        registerFactory(NamedScreen.ACCESSIBILITY_SETTINGS, () -> {
            return new AccessibilityOptionsScreen(Minecraft.getInstance().gui.screen(), Minecraft.getInstance().options);
        });
        registerFactory(NamedScreen.OPEN_LAN_WORLD, MultiplayerOptionsScreen::new);
        registerFactory(NamedScreen.FRIENDS, FriendsOverlayScreen::new);
    }

    @Override // net.labymod.core.platform.PlatformScreenHandler
    protected void registerFactory(GameScreen screen, Function<Screen, Screen> screenFactory) {
        registerFactory(screen, () -> {
            return (Screen) screenFactory.apply(Minecraft.getInstance().gui.screen());
        });
    }
