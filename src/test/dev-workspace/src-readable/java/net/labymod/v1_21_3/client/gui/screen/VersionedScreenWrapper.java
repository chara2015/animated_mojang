...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_3/client/gui/screen/VersionedScreenWrapper.class */
public class VersionedScreenWrapper implements ScreenWrapper {
    private final fty screen;
    private final fns graphics;
    private Metadata metadata;
    private boolean initialized;

    public VersionedScreenWrapper(fty screen) {
        if (screen == null) {
            throw new NullPointerException("screen is null");
        }
        fmg minecraft = fmg.Q();
        this.graphics = new fns(minecraft, minecraft.aR().c());
        this.screen = screen;
    ...
    public void initialize(Parent parent) {
        if (parent != null) {
            this.screen.b(fmg.Q(), (int) parent.bounds().getWidth(BoundsType.INNER), (int) parent.bounds().getHeight(BoundsType.INNER));
        } else {
            Window window = Laby.labyAPI().minecraft().minecraftWindow();
            this.screen.b(fmg.Q(), window.getScaledWidth(), window.getScaledHeight());
        }
    ...
    public void resize(int width, int height) {
        this.screen.b(fmg.Q(), width, height);
        this.initialized = true;
    }

    @Override // net.labymod.api.client.gui.screen.ScreenInstance
    public void render(ScreenContext context) {
        this.graphics.setPose((fgs) context.stack().getProvider().getPoseStack());
