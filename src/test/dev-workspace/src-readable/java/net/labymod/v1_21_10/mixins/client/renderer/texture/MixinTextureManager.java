package net.labymod.v1_21_10.mixins.client.renderer.texture;

import java.util.Map;
import net.labymod.api.client.resources.ResourceLocation;
import net.labymod.api.util.CastUtil;
import net.labymod.core.client.resources.texture.GameTextureManager;
import net.labymod.laby3d.api.RenderDeviceException;
import net.labymod.laby3d.api.RenderDeviceOutOfMemoryException;
import net.labymod.v1_21_10.client.util.MinecraftUtil;
import org.spongepowered.asm.mixin.Final;
import org.spongepowered.asm.mixin.Mixin;
import org.spongepowered.asm.mixin.Mutable;
import org.spongepowered.asm.mixin.Shadow;

/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_10/mixins/client/renderer/texture/MixinTextureManager.class */
@Mixin({icq.class})
public abstract class MixinTextureManager implements GameTextureManager {

    @Shadow
    @Mutable
    @Final
    private Map<amj, iby> c;

    @Shadow
    public abstract void a(amj amjVar, iby ibyVar);

    @Shadow
    public abstract void a(amj amjVar, icf icfVar);

    @Override // net.labymod.core.client.resources.texture.GameTextureManager
    public boolean hasResource(ResourceLocation location) {
        return this.c.get((amj) location) != null;
    }

    @Override // net.labymod.core.client.resources.texture.GameTextureManager
    public void registerAndRelease(ResourceLocation location, Object texture) {
        Object texture2 = MinecraftUtil.toMinecraft(texture);
        iby textureObject = (iby) CastUtil.requireInstanceOf(texture2, iby.class);
        iby currentTexture = this.c.get(location.getMinecraftLocation());
        if (texture2 instanceof icf) {
            icf reloadableTexture = (icf) texture2;
            a((amj) location.getMinecraftLocation(), reloadableTexture);
        } else {
            a((amj) location.getMinecraftLocation(), textureObject);
        }
        if (currentTexture != null) {
            try {
                currentTexture.close();
            } catch (RenderDeviceException | RenderDeviceOutOfMemoryException | IllegalStateException e) {
            }
        }
    }
}
