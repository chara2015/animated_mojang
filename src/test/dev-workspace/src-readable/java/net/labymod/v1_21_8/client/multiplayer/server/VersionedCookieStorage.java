...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_8/client/multiplayer/server/VersionedCookieStorage.class */
public class VersionedCookieStorage implements CookieStorage {
    private final gsd transferState;

    public VersionedCookieStorage(gsd transferState) {
    ...
    public void setCookie(ResourceLocation cookieId, byte[] data) {
        this.transferState.a().put((ame) cookieId.getMinecraftLocation(), data);
    }
    ...
    public Map<ResourceLocation, byte[]> cookies() {
        Map<ame, byte[]> vanillaCookies = this.transferState.a();
        Map<ResourceLocation, byte[]> cookies = new HashMap<>(vanillaCookies.size());
        for (Map.Entry<ame, byte[]> entry : vanillaCookies.entrySet()) {
