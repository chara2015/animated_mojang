...
import net.labymod.v1_19_4.laby3d.pipeline.LabyShaderInstance;
import org.spongepowered.asm.mixin.Final;
import org.spongepowered.asm.mixin.Mixin;
import org.spongepowered.asm.mixin.Shadow;
import org.spongepowered.asm.mixin.injection.At;

/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_19_4/mixins/client/renderer/MixinShaderInstance_Laby3DApi.class */
@Mixin({fir.class})
public class MixinShaderInstance_Laby3DApi {
    ...
    private add labyMod$rewriteId(String id, Operation<add> original) {
        if (this instanceof LabyShaderInstance) {
            return (add) LabyShaderInstance.rewriteId(id, this.F).getMinecraftLocation();
        }
        return (add) original.call(new Object[]{id});
    }

    @WrapOperation(method = {"getOrCreate"}, at = {@At(value = "NEW", target = "(Ljava/lang/String;)Lnet/minecraft/resources/ResourceLocation;")})
    private static add labyMod$rewriteProgramId(String id, Operation<add> original, @Local(argsOnly = true) String name) {
        if (name.contains(String.valueOf(':'))) {
            return (add) LabyShaderInstance.rewriteId(id, name).getMinecraftLocation();
        }
        return (add) original.call(new Object[]{id});
    }
