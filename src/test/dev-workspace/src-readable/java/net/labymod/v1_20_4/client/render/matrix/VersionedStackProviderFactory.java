...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_20_4/client/render/matrix/VersionedStackProviderFactory.class */
@Singleton
...
public class VersionedStackProviderFactory implements StackProviderFactory {
    ...
    public Stack create() {
        return Stack.create((StackProvider) new VersionedStackProvider(new eqb()));
    }

    @Override // net.labymod.api.client.render.matrix.StackProviderFactory
    public Stack create(Object poseStack) {
        Objects.requireNonNull(poseStack, "poseStack must not be null");
        if (!(poseStack instanceof eqb)) {
            throw new IllegalStateException(poseStack.getClass().getName() + " is not an instance of " + String.valueOf(eqb.class));
        }
        eqb stack = (eqb) poseStack;
