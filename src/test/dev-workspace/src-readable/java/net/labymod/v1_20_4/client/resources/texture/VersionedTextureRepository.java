...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_20_4/client/resources/texture/VersionedTextureRepository.class */
@Singleton
...
public class VersionedTextureRepository extends AbstractTextureRepository implements TextureRepository {
    ...
    public void onDevTick() {
        ...
        for (ResourceLocation resourceLocation : this.labyAPI.renderPipeline().resources().resourceLocationFactory().getCachedResourceLocations().values()) {
            if (resourceLocation instanceof PathResourceLocation) {
                ...
                ahg minecraftLocation = (ahg) resourceLocation.getMinecraftLocation();
    ...
    public void register(ResourceLocation location, Texture texture, TextureRepository.TextureRegistrationCallback callback) {
        gdy minecraftTexture = MinecraftUtil.toMinecraft(texture);
    ...
    public void preloadTexture(ResourceLocation location) {
        registerTexture(location, new geg((ahg) location.getMinecraftLocation()));
    }
    ...
    public Texture getTexture(ResourceLocation location) {
        ...
        Texture textureB = getTextureManager().b((ahg) location.getMinecraftLocation());
    ...
    private void release(ResourceLocation location, Runnable releaseHandler) {
        getTextureManager().c((ahg) location.getMinecraftLocation());
    ...
    private void registerTexture(@NotNull ResourceLocation location, gdy texture) {
        getTextureManager().a((ahg) location.getMinecraftLocation(), texture);
    }

    private void registerTexture(@NotNull ResourceLocation location, gdy texture, TextureRepository.TextureRegistrationCallback callback) {
    ...
    private geo getTextureManager() {
        if (this.textureManager == null) {
            this.textureManager = evi.O().Y();
        }
        return this.textureManager;
    }

    private Texture createTexture(gdy texture) {
