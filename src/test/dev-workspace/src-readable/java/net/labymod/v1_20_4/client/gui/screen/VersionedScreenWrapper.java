...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_20_4/client/gui/screen/VersionedScreenWrapper.class */
public class VersionedScreenWrapper implements ScreenWrapper {
    private final fdb screen;
    private final ewu graphics;
    private Metadata metadata;
    private boolean initialized;

    public VersionedScreenWrapper(fdb screen) {
        if (screen == null) {
            throw new NullPointerException("screen is null");
        }
        evi minecraft = evi.O();
        this.graphics = new ewu(minecraft, minecraft.aO().c());
        this.screen = screen;
    ...
    public void initialize(Parent parent) {
        if (parent != null) {
            this.screen.b(evi.O(), (int) parent.bounds().getWidth(BoundsType.INNER), (int) parent.bounds().getHeight(BoundsType.INNER));
        } else {
            Window window = Laby.labyAPI().minecraft().minecraftWindow();
            this.screen.b(evi.O(), window.getScaledWidth(), window.getScaledHeight());
        }
    ...
    public void resize(int width, int height) {
        this.screen.b(evi.O(), width, height);
        this.initialized = true;
    }

    @Override // net.labymod.api.client.gui.screen.ScreenInstance
    public void render(ScreenContext context) {
        this.graphics.setPose((eqb) context.stack().getProvider().getPoseStack());
