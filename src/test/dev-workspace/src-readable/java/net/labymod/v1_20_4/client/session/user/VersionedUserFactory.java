...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_20_4/client/session/user/VersionedUserFactory.class */
@Singleton
...
public class VersionedUserFactory implements UserFactory {
    ...
    public evx createUser(Session session) {
        UUID newUuidObject = UUID.fromString(session.getUniqueId().toString());
        return new evx(session.getUsername(), newUuidObject, session.getAccessToken(), Optional.empty(), Optional.empty(), session.isPremium() ? a.c : a.a);
    }
}
