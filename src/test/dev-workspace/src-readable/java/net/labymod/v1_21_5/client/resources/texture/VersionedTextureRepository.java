...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_5/client/resources/texture/VersionedTextureRepository.class */
@Singleton
...
public class VersionedTextureRepository extends AbstractTextureRepository implements TextureRepository {
    ...
    public void onDevTick() {
        ...
        for (ResourceLocation resourceLocation : this.labyAPI.renderPipeline().resources().resourceLocationFactory().getCachedResourceLocations().values()) {
            if (resourceLocation instanceof PathResourceLocation) {
                ...
                alr minecraftLocation = (alr) resourceLocation.getMinecraftLocation();
    ...
    public void register(ResourceLocation location, Texture texture, TextureRepository.TextureRegistrationCallback callback) {
        hkb minecraftTexture = MinecraftUtil.toMinecraft(texture);
    ...
    public void preloadTexture(ResourceLocation location) {
        registerTexture(location, new hki((alr) location.getMinecraftLocation()));
    }
    ...
    public Texture getTexture(ResourceLocation location) {
        ...
        Texture textureB = getTextureManager().b((alr) location.getMinecraftLocation());
    ...
    private void release(ResourceLocation location, Runnable releaseHandler) {
        getTextureManager().c((alr) location.getMinecraftLocation());
    ...
    private void registerTexture(@NotNull ResourceLocation location, hkb texture) {
        hks textureManager = getTextureManager();
        if (texture instanceof hkh) {
            hkh reloadableTexture = (hkh) texture;
    ...
    private void registerTexture(ResourceLocation location, hkb texture, TextureRepository.TextureRegistrationCallback callback) {
    ...
    private Texture createTexture(hkb texture) {
