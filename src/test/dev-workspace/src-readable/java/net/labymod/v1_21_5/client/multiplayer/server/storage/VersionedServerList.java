...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_5/client/multiplayer/server/storage/VersionedServerList.class */
public class VersionedServerList extends DefaultServerList {
    ...
    public void load() {
        try {
            ...
            ua root = un.a(this.gameDirectory.toPath().resolve("servers.dat"));
            if (root == null) {
                return;
            } else {
    ...
    private ua serverDataToTag(StorageServerData data) {
        ua tag = new ua();
    ...
    private StorageServerData tagToServerData(ua tag) throws MatchException {
        String str;
        ...
        if (tag.b("labyModMetadata")) {
            ua metaTag = tag.n("labyModMetadata");
            for (String key : metaTag.e()) {
                tx txVarA = metaTag.a(key);
                if (txVarA instanceof uy) {
                    try {
                        String strValue = ((uy) txVarA).k();
                        str = strValue;
                    } catch (Throwable th) {
                        throw new MatchException(th.toString(), th);
                    }
                } else if (txVarA instanceof tx) {
                    tx byteArrayTag = txVarA;
