...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_21_5/client/multiplayer/server/VersionedCookieStorage.class */
public class VersionedCookieStorage implements CookieStorage {
    private final gmh transferState;

    public VersionedCookieStorage(gmh transferState) {
    ...
    public void setCookie(ResourceLocation cookieId, byte[] data) {
        this.transferState.a().put((alr) cookieId.getMinecraftLocation(), data);
    }
    ...
    public Map<ResourceLocation, byte[]> cookies() {
        Map<alr, byte[]> vanillaCookies = this.transferState.a();
        Map<ResourceLocation, byte[]> cookies = new HashMap<>(vanillaCookies.size());
        for (Map.Entry<alr, byte[]> entry : vanillaCookies.entrySet()) {
