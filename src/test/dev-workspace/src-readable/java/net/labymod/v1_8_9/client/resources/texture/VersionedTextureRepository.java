...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_8_9/client/resources/texture/VersionedTextureRepository.class */
@Singleton
...
public class VersionedTextureRepository extends AbstractTextureRepository implements TextureRepository {
    ...
    public void register(ResourceLocation location, Texture texture, TextureRepository.TextureRegistrationCallback callback) {
        bmk minecraftTexture = MinecraftUtil.toMinecraft(texture);
    ...
    public void onDevTick() {
        ...
        for (ResourceLocation resourceLocation : this.labyAPI.renderPipeline().resources().resourceLocationFactory().getCachedResourceLocations().values()) {
            if (resourceLocation instanceof PathResourceLocation) {
                ...
                jy minecraftLocation = (jy) resourceLocation.getMinecraftLocation();
    ...
    public void preloadTexture(ResourceLocation location) {
        ...
        if (minecraft.isOnRenderThread()) {
            registerTexture(location, new bme((jy) location.getMinecraftLocation()));
        } else {
    ...
    public Texture getTexture(ResourceLocation location) {
        ...
        VersionedLabyTexture versionedLabyTextureB = getTextureManager().b((jy) location.getMinecraftLocation());
    ...
    public Texture getOrCreateTexture(ResourceLocation location) {
        ...
        unqueueTexture(location);
        bmk texture = getTextureManager().b((jy) location.getMinecraftLocation());
        if (texture == null) {
            texture = new bme((jy) location.getMinecraftLocation());
            getTextureManager().a((jy) location.getMinecraftLocation(), texture);
        }
    ...
    private void registerTexture(@NotNull ResourceLocation location, bmk texture) {
        getTextureManager().a((jy) location.getMinecraftLocation(), texture);
    }

    private void registerTexture(@NotNull ResourceLocation location, bmk texture, TextureRepository.TextureRegistrationCallback callback) {
