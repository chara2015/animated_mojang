...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_16_5/mixins/server/network/MixinServerGamePacketListenerImpl.class */
@Mixin({aay.class})
public class MixinServerGamePacketListenerImpl {
    ...
    private void labyMod$fireDisconnectEventPre(nr reason, InsertInfo ci) {
        GameProfile profile = this.b.eA();
        Laby.fireEvent(new IntegratedServerPlayerDisconnectEvent(Phase.PRE, profile));
    }

    @Insert(method = {"onDisconnect"}, at = @At("TAIL"))
    private void labyMod$fireDisconnectEventPost(nr reason, InsertInfo ci) {
        GameProfile profile = this.b.eA();
