...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_16_5/mixins/client/world/chunk/MixinChunkAccess.class */
@Mixin({cfw.class})
public interface MixinChunkAccess extends Chunk {
    ...
    default int getAbsoluteBlockX(int chunkBlockX) {
        return (getChunkX() << 4) + chunkBlockX;
    }

    @Override // net.labymod.api.client.world.chunk.Chunk
    default int getAbsoluteBlockZ(int chunkBlockZ) {
        return (getChunkZ() << 4) + chunkBlockZ;
    }
    ...
    default BlockEntity[] getBlockEntities() {
        if (this instanceof cgh) {
            cgh levelChunk = (cgh) this;
            Map<fx, ccj> map = levelChunk.y();
            BlockEntity[] blockEntities = new BlockEntity[map.size()];
            int index = 0;
            for (Map.Entry<fx, ccj> entry : map.entrySet()) {
                int i = index;
                index++;
