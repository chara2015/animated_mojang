...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_16_5/client/resources/pack/VersionedResourcePackRepository.class */
@Singleton
...
public class VersionedResourcePackRepository extends AbstractResourcePackRepository<abu> {
    ...
    public void registerSilentPack(ResourcePack pack) {
        ...
        SilentReloadableResourceManager silentReloadableResourceManagerN = djz.C().N();
        if (silentReloadableResourceManagerN instanceof acm) {
            ((acm) silentReloadableResourceManagerN).loadSilent(pack);
    ...
    public void onRebuildSelected(List<abu> selected) {
        this.selectedPacks.clear();
        setHasServerPackSelected(false);
        for (abu pack : selected) {
            this.selectedPacks.add(pack.e());
            checkServerPack(pack);
        }
    }

    private void checkServerPack(abu pack) {
        abj resources = pack.d();
        try {
    ...
    public PackSelectionModel createSelectionModel(@NotNull Consumer<PackSelectionModel> onListChange) {
        PackSelectionModel model = new drh(() -> {
        }, pack -> {
            return null;
        }, djz.C().O(), packRepository -> {
