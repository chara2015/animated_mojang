...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_16_5/client/resources/texture/VersionedTextureRepository.class */
@Singleton
...
public class VersionedTextureRepository extends AbstractTextureRepository implements TextureRepository {
    ...
    public void register(ResourceLocation location, Texture texture, TextureRepository.TextureRegistrationCallback callback) {
        ejq minecraftTexture = MinecraftUtil.toMinecraft(texture);
    ...
    public void onDevTick() {
        ...
        for (ResourceLocation resourceLocation : this.labyAPI.renderPipeline().resources().resourceLocationFactory().getCachedResourceLocations().values()) {
            if (resourceLocation instanceof PathResourceLocation) {
                ...
                vk minecraftLocation = (vk) resourceLocation.getMinecraftLocation();
    ...
    public void preloadTexture(ResourceLocation location) {
        if (RenderSystem.isOnRenderThread()) {
            registerTexture(location, new PreloadedTexture((vk) location.getMinecraftLocation()));
        } else {
            ThreadSafe.executeOnRenderThread(() -> {
                registerTexture(location, new PreloadedTexture((vk) location.getMinecraftLocation()));
            });
    ...
    public Texture getTexture(ResourceLocation location) {
        ...
        Texture textureB = getTextureManager().b((vk) location.getMinecraftLocation());
    ...
    public Texture getOrCreateTexture(ResourceLocation location) {
        ...
        unqueueTexture(location);
        ekd textureManager = getTextureManager();
        vk minecraftLocation = (vk) location.getMinecraftLocation();
        ejq texture = textureManager.b(minecraftLocation);
        if (texture == null) {
            textureManager.a(minecraftLocation);
    ...
    private void release(ResourceLocation location, Runnable releaseHandler) {
        getTextureManager().c((vk) location.getMinecraftLocation());
    ...
    private void registerTexture(@NotNull ResourceLocation location, ejq texture) {
        getTextureManager().a((vk) location.getMinecraftLocation(), texture);
    }

    private void registerTexture(@NotNull ResourceLocation location, ejq texture, TextureRepository.TextureRegistrationCallback callback) {
    ...
    private Texture createTexture(ejq texture) {
