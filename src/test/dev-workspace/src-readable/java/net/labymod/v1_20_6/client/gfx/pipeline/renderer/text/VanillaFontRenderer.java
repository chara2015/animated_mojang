...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_20_6/client/gfx/pipeline/renderer/text/VanillaFontRenderer.class */
@Singleton
...
public class VanillaFontRenderer implements MinecraftFontRenderer {
    ...
    public void render(Matrix4f pose, Component text, float x, float y, int argb, int packedLightCoords, int backgroundArgb, int flags) {
        fgr font = getFont();
        font.a((xp) this.componentMapper.toMinecraftComponent(text), x, y, argb, FontFlags.isShadow(flags), pose, getBufferSource(), getDisplayMode(flags), backgroundArgb, packedLightCoords);
    }
    ...
    public float getWidth(Component text) {
        return getFont().a((xp) this.componentMapper.toMinecraftComponent(text));
    }
    ...
    public String trimTextToWidth(String text, float maxWidth, StringStart start) {
        ffs splitter = getFont().b();
        if (start == StringStart.LEFT) {
            return splitter.b(text, MathHelper.ceil(maxWidth), ym.a);
        }
        return splitter.c(text, MathHelper.ceil(maxWidth), ym.a);
    }

    @Override // net.labymod.api.client.gfx.pipeline.renderer.text.FontRenderer
    public float getWidth(int codepoint, Style style) {
        return StringSplitterAccessor.cast(getFont().b()).getWidthProvider().getWidth(codepoint, (ym) CastUtil.cast(style));
    }

    private gdq getBufferSource() {
        return ffh.Q().aQ().c();
    }
    ...
    /* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_20_6/client/gfx/pipeline/renderer/text/VanillaFontRenderer$PreparedTextBuilder.class */
    public static class PreparedTextBuilder implements aym {
        ...
        public PreparedTextBuilder(fgr font, float x, float y, int color, boolean drawShadow, int backgroundColor) {
        ...
        public boolean accept(int position, @NotNull ym style, int codepoint) {
            fkb fkbVarA;
            fjx fontSet = this.accessor.labyMod$getFontSet(style.k());
            eyc glyphInfo = fontSet.a(codepoint, this.accessor.labyMod$filterFishyGlyphs());
            if (style.f() && codepoint != 32) {
                fkbVarA = fontSet.a(glyphInfo);
            } else {
                fkbVarA = fontSet.a(codepoint);
            }
            fkb bakedGlyph = fkbVarA;
            boolean bold = style.b();
            yo textColor = style.a();
        ...
        private void addEffect(a effect) {
            fjx fontSet = this.accessor.labyMod$getFontSet(ym.b);
            fkb bakedGlyph = fontSet.b();
            this.builder.addEffect(new VanillaRenderedEffect(bakedGlyph, effect));
        }

        private int getTextColor(@Nullable yo textColor) {
            if (textColor != null) {
