package net.labymod.v1_12_2.laby3d.buffer;

import net.labymod.api.laby3d.buffer.GameVertexConsumer;
import net.labymod.api.util.color.format.ColorFormat;
import net.labymod.laby3d.api.vertex.VertexConsumer;
import org.jetbrains.annotations.NotNull;

/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_12_2/laby3d/buffer/VersionedGameVertexConsumer.class */
public class VersionedGameVertexConsumer implements GameVertexConsumer {
    private static final int MASK = 255;
    private static final int SHIFT = 16;
    private final buk delegate;
    private boolean vertexOpen;

    public VersionedGameVertexConsumer(buk delegate) {
        this.delegate = delegate;
    }

    @NotNull
    public VertexConsumer addVertex(float x, float y, float z) {
        if (this.vertexOpen) {
            this.delegate.d();
            this.vertexOpen = false;
        }
        this.delegate.b(x, y, z);
        this.vertexOpen = true;
        return this;
    }

    @NotNull
    public VertexConsumer setColor(int red, int green, int blue, int alpha) {
        this.delegate.b(red, green, blue, alpha);
        return this;
    }

    @NotNull
    public VertexConsumer setColor(float red, float green, float blue, float alpha) {
        this.delegate.a(red, green, blue, alpha);
        return this;
    }

    @NotNull
    public VertexConsumer setColor(int argb) {
        ColorFormat format = ColorFormat.ARGB32;
        int red = format.red(argb);
        int green = format.green(argb);
        int blue = format.blue(argb);
        int alpha = format.alpha(argb);
        this.delegate.b(red, green, blue, alpha);
        return this;
    }

    @NotNull
    public VertexConsumer setUv(float u, float v) {
        this.delegate.a(u, v);
        return this;
    }

    @NotNull
    public VertexConsumer setPackedLight(int lightCoords) {
        short u = (short) (lightCoords & 255);
        short v = (short) ((lightCoords >> 16) & 255);
        this.delegate.a(u, v);
        return this;
    }

    @NotNull
    public VertexConsumer setPackedOverlay(int overlayCoords) {
        return this;
    }

    @NotNull
    public VertexConsumer setUv(short i, short i1) {
        return this;
    }

    @NotNull
    public VertexConsumer setNormal(float x, float y, float z) {
        this.delegate.c(x, y, z);
        return this;
    }

    @NotNull
    public VertexConsumer setInt(int i) {
        return this;
    }

    @NotNull
    public VertexConsumer setShort(short i) {
        return this;
    }

    @NotNull
    public VertexConsumer setFloat(float v) {
        return this;
    }

    @Override // net.labymod.api.laby3d.buffer.GameVertexConsumer
    public void flushPendingVertex() {
        if (this.vertexOpen) {
            this.delegate.d();
            this.vertexOpen = false;
        }
    }
}
