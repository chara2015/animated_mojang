...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_17_1/client/render/matrix/VersionedStackProviderFactory.class */
@Singleton
...
public class VersionedStackProviderFactory implements StackProviderFactory {
    ...
    public Stack create() {
        return Stack.create((StackProvider) new VersionedStackProvider(new dql()));
    }

    @Override // net.labymod.api.client.render.matrix.StackProviderFactory
    public Stack create(Object poseStack) {
        Objects.requireNonNull(poseStack, "poseStack must not be null");
        if (!(poseStack instanceof dql)) {
            throw new IllegalStateException(poseStack.getClass().getName() + " is not an instance of " + String.valueOf(dql.class));
        }
        dql stack = (dql) poseStack;
