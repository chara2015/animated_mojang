...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_17_1/mixins/client/screen/MixinScreenTooltip.class */
@Mixin({eaq.class})
public abstract class MixinScreenTooltip {
    @Shadow
    protected abstract void a(dql dqlVar, List<edb> list, int i, int i2);
    ...
    private void labyMod$renderTooltip(eaq instance, dql stack, List<edb> components, int x, int y) {
    ...
    private void labyMod$renderTooltipCharSequence(eaq instance, dql stack, List<edb> components, int x, int y) {
        labyMod$renderTooltipInternal(stack, components, x, y);
    }

    private void labyMod$renderTooltipInternal(dql stack, List<edb> components, int x, int y) {
