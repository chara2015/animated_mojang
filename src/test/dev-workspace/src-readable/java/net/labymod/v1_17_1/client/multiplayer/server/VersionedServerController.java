...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_17_1/client/multiplayer/server/VersionedServerController.class */
@Singleton
...
public class VersionedServerController extends DefaultAbstractServerController {
    ...
    public ServerData createServerData(Object mojangServerData) {
        ...
        ejn serverData = (ejn) mojangServerData;
    ...
    private void sendCustomPayload(@NotNull ejj connection, @NotNull ResourceLocation identifier, byte[] payload) {
        connection.a(new ty((ww) identifier.getMinecraftLocation(), new og(Unpooled.wrappedBuffer(payload))));
    }

    private Component fromMinecraftComponent(os component) {
        if (component == null) {
    ...
    private List<Component> fromMinecraftComponentCollection(@Nullable List<os> list) {
        if (list == null || list.isEmpty()) {
            return new ArrayList();
        }
        List<Component> result = new ArrayList<>(list.size());
        for (os component : list) {
    ...
    public void leaveServer() {
        ...
        minecraft.r.U();
        if (singleplayer) {
            minecraft.b(new eaa((os) this.componentMapper.toMinecraftComponent(Component.translatable("menu.savingLevel", new Component[0]))));
        } else {
            minecraft.r();
        }
        eaq eavVar = new eav();
        if (singleplayer) {
            ...
            display = new dqv(eavVar);
        } else {
            display = new edc(eavVar);
        }
        minecraft.a(display);
    }
    ...
    public void joinServer(@NotNull JoinServerRequest request) {
        ...
        if (requestedScreen instanceof eaq) {
            eaq screenInstance = (eaq) requestedScreen;
            screen = screenInstance;
        }
        dzq.a(screen, dvp.C(), ejt.a(serverData.b), serverData);
    }
    ...
    public ScreenWrapper createEditServerScreen(StorageServerData data, Consumer<StorageServerData> callback) {
        ...
        ejn mcData = new ejn(data.getName(), data.address().toString(), data.isLan());
        mcData.a(a.values()[data.resourcePackStatus().ordinal()]);
        dzy screen = new dzy(prevScreen, addServer -> {
            if (addServer) {
