...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_17_1/client/gfx/pipeline/renderer/text/VanillaFontRenderer.class */
@Singleton
...
public class VanillaFontRenderer implements MinecraftFontRenderer {
    ...
    public void render(Matrix4f pose, Component text, float x, float y, int argb, int packedLightCoords, int backgroundArgb, int flags) {
        dwl font = getFont();
        font.a((os) this.componentMapper.toMinecraftComponent(text), x, y, argb, FontFlags.isShadow(flags), (d) this.mathMapper.toMatrix4f(pose), getBufferSource(), getDisplayMode(flags), backgroundArgb, packedLightCoords);
    }
    ...
    public float getWidth(Component text) {
        return getFont().a((os) this.componentMapper.toMinecraftComponent(text));
    }
    ...
    public String trimTextToWidth(String text, float maxWidth, StringStart start) {
        dwa splitter = getFont().b();
        if (start == StringStart.LEFT) {
            return splitter.b(text, MathHelper.ceil(maxWidth), pc.a);
        }
        return splitter.c(text, MathHelper.ceil(maxWidth), pc.a);
    }

    @Override // net.labymod.api.client.gfx.pipeline.renderer.text.FontRenderer
    public float getWidth(int codepoint, Style style) {
        return StringSplitterAccessor.cast(getFont().b()).getWidthProvider().getWidth(codepoint, (pc) CastUtil.cast(style));
    }

    private eni getBufferSource() {
        return dvp.C().aE().b();
    }
    ...
    /* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_17_1/client/gfx/pipeline/renderer/text/VanillaFontRenderer$PreparedTextBuilder.class */
    public static class PreparedTextBuilder implements agt {
        ...
        public PreparedTextBuilder(dwl font, float x, float y, int color, boolean drawShadow, int backgroundColor) {
        ...
        public boolean accept(int position, @NotNull pc style, int codepoint) {
            dyp dypVarB;
            dym fontSet = this.accessor.labyMod$getFontSet(style.k());
            dop glyphInfo = fontSet.a(codepoint);
            if (style.f() && codepoint != 32) {
                dypVarB = fontSet.a(glyphInfo);
            } else {
                dypVarB = fontSet.b(codepoint);
            }
            dyp bakedGlyph = dypVarB;
            boolean bold = style.b();
            pe textColor = style.a();
        ...
        private void addEffect(a effect) {
            dym fontSet = this.accessor.labyMod$getFontSet(pc.b);
            dyp bakedGlyph = fontSet.a();
            this.builder.addEffect(new VanillaRenderedEffect(bakedGlyph, effect));
        }

        private int getTextColor(@Nullable pe textColor) {
            if (textColor != null) {
