...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_17_1/client/resources/texture/VersionedTextureRepository.class */
@Singleton
...
public class VersionedTextureRepository extends AbstractTextureRepository implements TextureRepository {
    ...
    public void onDevTick() {
        ...
        for (ResourceLocation resourceLocation : this.labyAPI.renderPipeline().resources().resourceLocationFactory().getCachedResourceLocations().values()) {
            if (resourceLocation instanceof PathResourceLocation) {
                ...
                ww minecraftLocation = (ww) resourceLocation.getMinecraftLocation();
    ...
    public void register(ResourceLocation location, Texture texture, TextureRepository.TextureRegistrationCallback callback) {
        exa minecraftTexture = MinecraftUtil.toMinecraft(texture);
    ...
    public void preloadTexture(ResourceLocation location) {
        registerTexture(location, new exi((ww) location.getMinecraftLocation()));
    }
    ...
    public Texture getTexture(ResourceLocation location) {
        ...
        Texture textureB = getTextureManager().b((ww) location.getMinecraftLocation());
    ...
    private void release(ResourceLocation location, Runnable releaseHandler) {
        getTextureManager().c((ww) location.getMinecraftLocation());
    ...
    private void registerTexture(@NotNull ResourceLocation location, exa texture) {
        getTextureManager().a((ww) location.getMinecraftLocation(), texture);
    }

    private void registerTexture(@NotNull ResourceLocation location, exa texture, TextureRepository.TextureRegistrationCallback callback) {
    ...
    private exn getTextureManager() {
        if (this.textureManager == null) {
            this.textureManager = dvp.C().M();
        }
        return this.textureManager;
    }

    private Texture createTexture(exa texture) {
