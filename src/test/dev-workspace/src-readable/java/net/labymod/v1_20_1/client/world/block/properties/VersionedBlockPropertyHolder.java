...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_20_1/client/world/block/properties/VersionedBlockPropertyHolder.class */
public class VersionedBlockPropertyHolder extends AbstractBlockPropertyHolder {
    private final dcd<?, ?> holder;

    public VersionedBlockPropertyHolder(dcd<?, ?> holder) {
        this.holder = holder;
    }
    ...
    protected <T extends Comparable<T>> T getPropertyValue(BlockProperty blockProperty) {
        dde ddeVar = (dde) blockProperty;
        if (!this.holder.b(ddeVar)) {
            return null;
        }
        Enum r0 = (T) this.holder.c(ddeVar);
    ...
    public boolean hasProperty(BlockProperty property) {
        return this.holder.b((dde) property);
    }
}
