...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_20_1/client/session/VersionedSessionAccessor.class */
@Singleton
...
public class VersionedSessionAccessor extends DefaultAbstractSessionAccessor<eoc> {
    ...
    public eoc toGameSession(Session session) {
        return (eoc) this.userFactory.createUser(session);
    }

    @Override // net.labymod.core.client.session.DefaultAbstractSessionAccessor
    public Session fromGameSession(eoc gameSession) {
        if (gameSession == null) {
            return null;
        }
        return createSession(gameSession.c(), gameSession.h().getId(), gameSession.d());
    }
