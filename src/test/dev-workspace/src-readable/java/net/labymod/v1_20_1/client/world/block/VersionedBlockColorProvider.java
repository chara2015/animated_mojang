...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_20_1/client/world/block/VersionedBlockColorProvider.class */
@Singleton
...
public class VersionedBlockColorProvider implements BlockColorProvider {
    ...
    public int getColor(BlockState state) {
        acq blockId = jb.f.b(state.block());
    ...
    public int getColorMultiplier(BlockState state, int minX, int minZ, int maxX, int maxZ) {
        ...
        return blockColors.a((dcb) state, this.biomeBlender.setLevel(cmmVar).setArea(minX, minZ, maxX, maxZ), toBlockPos(state.position()), 0);
    }

    private gu toBlockPos(IntVector3 vec) {
