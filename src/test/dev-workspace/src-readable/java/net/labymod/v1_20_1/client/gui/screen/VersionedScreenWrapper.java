...
/* JADX INFO: loaded from: LabyMod-4.jar:net/labymod/v1_20_1/client/gui/screen/VersionedScreenWrapper.class */
public class VersionedScreenWrapper implements ScreenWrapper {
    private final euq screen;
    private final eox graphics;
    private Metadata metadata;
    private boolean initialized;

    public VersionedScreenWrapper(euq screen) {
        if (screen == null) {
            throw new NullPointerException("screen is null");
        }
        enn minecraft = enn.N();
        this.graphics = new eox(minecraft, minecraft.aN().b());
        this.screen = screen;
    ...
    public void initialize(Parent parent) {
        if (parent != null) {
            this.screen.b(enn.N(), (int) parent.bounds().getWidth(BoundsType.INNER), (int) parent.bounds().getHeight(BoundsType.INNER));
        } else {
            Window window = Laby.labyAPI().minecraft().minecraftWindow();
            this.screen.b(enn.N(), window.getScaledWidth(), window.getScaledHeight());
        }
    ...
    public void resize(int width, int height) {
        this.screen.b(enn.N(), width, height);
        this.initialized = true;
    }

    @Override // net.labymod.api.client.gui.screen.ScreenInstance
    public void render(ScreenContext context) {
        this.graphics.setPose((eij) context.stack().getProvider().getPoseStack());
        MutableMouse mouse = context.mouse();
        this.screen.b(this.graphics, mouse.getX(), mouse.getY(), context.getTickDelta());
    }
