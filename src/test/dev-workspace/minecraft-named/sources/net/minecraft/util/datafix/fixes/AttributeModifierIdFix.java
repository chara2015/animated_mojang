...
/* JADX INFO: loaded from: minecraft-client-1.21.11-named.jar:net/minecraft/util/datafix/fixes/AttributeModifierIdFix.class */
public class AttributeModifierIdFix extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        return TypeRewriteRule.seq(fixTypeEverywhereTyped("AttributeIdFix (ItemStack)", $$0, $$12 -> {
            return $$12.updateTyped($$1, $$02 -> {
                return $$02.update(DSL.remainderFinder(), AttributeModifierIdFix::fixItemStackComponents);
            });
        }), new TypeRewriteRule[]{fixTypeEverywhereTyped("AttributeIdFix (Entity)", getInputSchema().getType(References.ENTITY), AttributeModifierIdFix::fixEntity), fixTypeEverywhereTyped("AttributeIdFix (Player)", getInputSchema().getType(References.PLAYER), AttributeModifierIdFix::fixEntity)});
    }
    ...
    private static Stream<Dynamic<?>> fixModifiers(Stream<Dynamic<?>> $$0) {
        ...
        $$0.forEach($$1 -> {
            UUID $$2 = uuidFromIntArray($$1.get("uuid").asIntStream().toArray());
            String $$3 = $$1.get(JigsawBlockEntity.NAME).asString("");
            String $$4 = $$2 != null ? ID_MAP.get($$2) : null;
            String $$5 = NAME_MAP.get($$3);
            if ($$4 != null) {
                object2ObjectArrayMap.put($$4, $$1.set(Entity.TAG_ID, $$1.createString($$4)).remove("uuid").remove(JigsawBlockEntity.NAME));
                return;
            }
            if ($$5 != null) {
                Dynamic<?> $$6 = (Dynamic) object2ObjectArrayMap.get($$5);
                if ($$6 == null) {
                    object2ObjectArrayMap.put($$5, $$1.set(Entity.TAG_ID, $$1.createString($$5)).remove("uuid").remove(JigsawBlockEntity.NAME));
                    return;
                }
                double $$7 = $$6.get("amount").asDouble(Density.SURFACE);
                double $$8 = $$1.get("amount").asDouble(Density.SURFACE);
                object2ObjectArrayMap.put($$5, $$6.set("amount", $$1.createDouble($$7 + $$8)));
                return;
            }
            String $$9 = "minecraft:" + ($$2 != null ? $$2.toString().toLowerCase(Locale.ROOT) : "unknown");
            object2ObjectArrayMap.put($$9, $$1.set(Entity.TAG_ID, $$1.createString($$9)).remove("uuid").remove(JigsawBlockEntity.NAME));
        });
        return object2ObjectArrayMap.values().stream();
    }

    private static Dynamic<?> convertModifierForEntity(Dynamic<?> $$0) {
        return $$0.renameField(Entity.TAG_UUID, "uuid").renameField(StateHolder.NAME_TAG, JigsawBlockEntity.NAME).renameField("Amount", "amount").renameAndFixField("Operation", "operation", $$02 -> {
            String str;
            switch ($$02.asInt(0)) {
                case 0:
                    str = "add_value";
            ...
            return $$02.createString(str);
        });
    }

    private static Dynamic<?> fixItemStackComponents(Dynamic<?> $$0) {
        return $$0.update("minecraft:attribute_modifiers", $$02 -> {
            return $$02.update("modifiers", $$02 -> {
                Optional map = $$02.asStreamOpt().result().map(AttributeModifierIdFix::fixModifiersTypeWrapper);
    ...
    private static Dynamic<?> fixAttribute(Dynamic<?> $$0) {
        return $$0.renameField(StateHolder.NAME_TAG, Entity.TAG_ID).renameField("Base", "base").renameAndFixField("Modifiers", "modifiers", $$1 -> {
            Optional map = $$1.asStreamOpt().result().map($$02 -> {
                return $$02.map(AttributeModifierIdFix::convertModifierForEntity);
            }).map(AttributeModifierIdFix::fixModifiersTypeWrapper);
            Objects.requireNonNull($$0);
            return (Dynamic) DataFixUtils.orElse(map.map($$0::createList), $$1);
        });
    }

    private static Typed<?> fixEntity(Typed<?> $$0) {
        return $$0.update(DSL.remainderFinder(), $$02 -> {
            return $$02.renameAndFixField("Attributes", LivingEntity.TAG_ATTRIBUTES, $$02 -> {
                Optional map = $$02.asStreamOpt().result().map($$02 -> {
                    return $$02.map(AttributeModifierIdFix::fixAttribute);
                });
                Objects.requireNonNull($$02);
                return (Dynamic) DataFixUtils.orElse(map.map($$02::createList), $$02);
            });
