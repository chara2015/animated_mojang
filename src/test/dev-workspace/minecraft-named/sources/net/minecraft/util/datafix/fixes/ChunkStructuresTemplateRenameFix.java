...
/* JADX INFO: loaded from: minecraft-client-1.21.11-named.jar:net/minecraft/util/datafix/fixes/ChunkStructuresTemplateRenameFix.class */
public class ChunkStructuresTemplateRenameFix extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        Type<?> $$0 = getInputSchema().getType(References.STRUCTURE_FEATURE);
        return fixTypeEverywhereTyped("ChunkStructuresTemplateRenameFix", $$0, $$02 -> {
            return $$02.update(DSL.remainderFinder(), this::fixChildren);
        });
    }

    private Dynamic<?> fixChildren(Dynamic<?> $$0) {
        return $$0.update("Children", $$1 -> {
            return $$0.createList($$1.asStream().map($$1 -> {
                return fixTag($$0, $$1);
            }));
        });
    }
    ...
    private Dynamic<?> fixTag(Dynamic<?> $$0, Dynamic<?> $$1) {
        String $$2 = $$0.get(Entity.TAG_ID).asString("");
        if (RENAMES.containsKey($$2)) {
            Pair<String, ImmutableMap<String, String>> $$3 = (Pair) RENAMES.get($$2);
            if (((String) $$3.getFirst()).equals($$1.get(Entity.TAG_ID).asString(""))) {
