...
public final class VersionedSchematicScene {
	private static final String NORMAL_PATH = "assets/animated_mojang/data/normal_cave.schem";
	private static final VersionedSchematicScene EMPTY = new VersionedSchematicScene(1, 1, 1, new String[1], new float[1],
	...
	private static final Identifier BLOCK_TEXTURE = Identifier.fromNamespaceAndPath(
			AnimatedMojangClient.MOD_ID, "textures/gui/title/cave_blocks.png");
	private static final Identifier LAVA_TEXTURE = Identifier.fromNamespaceAndPath(
			AnimatedMojangClient.MOD_ID, "textures/gui/title/cave_lava_flow.png");
	private static final Identifier PARTICLE_TEXTURE = Identifier.fromNamespaceAndPath(
			AnimatedMojangClient.MOD_ID, "textures/gui/title/cave_particles_clean.png");
	...
	private final PerspectiveProjectionMatrixBuffer projectionBuffer = new PerspectiveProjectionMatrixBuffer("Animated Mojang cave projection");
	...
	private final GpuBuffer schematicUniform = RenderSystem.getDevice().createBuffer(
			() -> "Animated Mojang Schematic UBO", GpuBuffer.USAGE_UNIFORM | GpuBuffer.USAGE_COPY_DST,
			SCHEMATIC_UBO_SIZE);
	private final RenderTarget schematicTarget = new TextureTarget("Animated Mojang schematic target", 1, 1, true);

	private static VersionedSchematicScene loaded;

	private VersionedSchematicScene(int width, int height, int length, String[] blockStates, float[] blockLights,
	...
	public static VersionedSchematicScene get() {
		if (loaded == null) {
	...
	public void render(GuiGraphics graphics, float camera, float time) {
		...
		if (!blockRenderCache.isEmpty()) {
			VersionGuiRenderBridge.submit(graphics, new MeshRenderState(new Matrix3x2f(graphics.pose()), blockRenderCache,
					bounds, TextureSetup.singleTexture(blockTexture.getTextureView(), blockTexture.getSampler())));
		}
		if (!lavaRenderCache.isEmpty()) {
			VersionGuiRenderBridge.submit(graphics, new MeshRenderState(new Matrix3x2f(graphics.pose()), lavaRenderCache,
					bounds, TextureSetup.singleTexture(lavaTexture.getTextureView(), lavaTexture.getSampler())));
		}
	}

	public void renderDirect(float camera, float time, float openingTime) {
		renderDirect(cameraPose(camera, openingTime), time);
	}

	public void renderDirectAt(float x, float y, float z, float yaw, float pitch, float roll, float time, float openingTime) {
	...
	private void renderDirect(CameraPose pose, float time) {
		...
		try (RenderPass pass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(
				() -> "Animated Mojang schematic", target.getColorTextureView(), java.util.OptionalInt.empty(),
		...
		try (RenderPass pass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(
				() -> "Animated Mojang schematic linear blit", mainTarget.getColorTextureView(),
	...
	public void renderTorchEffects(GuiGraphics graphics, float camera, float time, float openingTime) {
		...
		try (ByteBufferBuilder byteBuffer = new ByteBufferBuilder(Math.max(1024, meshFaces.size() * 128))) {
			...
			try (meshData) {
				GpuBuffer vertexBuffer = RenderSystem.getDevice().createBuffer(
						() -> "Animated Mojang " + layer.name().toLowerCase() + " mesh",
	...
	private static void renderSamples(GuiGraphics graphics, List<BlockSample> samples, float block, float zoom,
	...
	private static VersionedSchematicScene load(String path) {
		try (InputStream stream = VersionedSchematicScene.class.getClassLoader().getResourceAsStream(path)) {
			if (stream == null) {
				return EMPTY;
			}
			...
			return new VersionedSchematicScene(width, height, length, samples.blockStates, samples.blockLights,
	...
	private static void renderGradient(GuiGraphics graphics, int width, int height, float time) {
